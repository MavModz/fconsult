<template>
    ...
    <Dialog v-model:visible="outModal" modal header="" :style="{'min-width': '350px'}"
        :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
        <div class="card-body">
            <div class="text-center">
                <center>
                ...
                <h3 class="fs-24 fw-semibold mb-1">Location Out Of Coverage</h3>
                <h6 class="fs-24 fw-semibold mb-1">Sorry We Are Serving in india only</h6>
        ...
        getAddress() {
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(
                    async (position) => {
                        await userGet(`/addbusiness-user-address/${position.coords.longitude}/${position.coords.latitude}`).then((r) => {
                            if(r.data.data.loc.country=='India'){
