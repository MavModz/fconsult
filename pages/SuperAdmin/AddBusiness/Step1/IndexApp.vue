...
export default {
  ...
  methods: {
    ...
    searchCompany() {
      ...
      this.timer = setTimeout(async () => {
        if (this.name !== null && this.name !== "") {
          await userGet(`/superadmin-search-company/${this.name}`).then((response) => {
    ...
    if(this.$route.query.company_id){
      ...
      await superadminGet(`/superadmin-check-company-name/${v.company_name}`).then((r) => {
        if (r.data.data > 0) {
    ...
    async addCompany(v) {
      ...
      if (this.$route.query.company_id) {
        ...
        await superadminPost('/superadmin-add-new-company', v).then(async (response) => {
