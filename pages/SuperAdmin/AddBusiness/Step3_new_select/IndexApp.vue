  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      const finalPayload = {
        services: payload,
        step: 3,
        superadmin_step:this.$route.query.company_id ? 4 : 1,
      };
      // return console.log("this.user.company_id",this.company._id)
      try {
        await superadminPost(`/superadmin-company-reapply/${this.company._id}`, finalPayload);
        this.$emit('nextPage'); // always emit nextPage
      } catch (error) {
        this.loading = false;
        console.log(error)
        await serverErrorMessage(() => this.handleSubmit());
        return;
      }
    ...
    async category_update() {
      ...
      try {
        await superadminPatch(`/company-update/${this.company._id}`, payload);
