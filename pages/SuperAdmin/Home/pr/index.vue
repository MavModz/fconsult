  ...
  <div class="card">
    <TabView class="card">
      <TabPanel header="Long Cards">
        <div class="mt-3 card-body">
          <div class="flex justify-end pb-4">
            <NuxtLink class="your-events-btn" to="pr/addLong">
      ...
      <TabPanel header="Short Cards">
        <div class="mt-3 card-body">
          <div class="flex justify-end pb-4">
            <NuxtLink class="your-events-btn" to="pr/addShort">
              <BootstrapIcon name="file-earmark-plus" size="2rem" />&nbsp; Add
            </NuxtLink>
          </div>
      ...
      <TabPanel header="Offers">
        <div class="mt-3 card-body">
          <DataTable :value="offers" tableStyle="min-width: 50rem">
            ...
            <Column header="Image">
              <template #body="slotProps">
                <img :src="slotProps?.data?.listing_id?.Permanent?.logo" class="w-6rem border-round img"
                  style="width: 100px" />
              </template>
            </Column>
            <Column header="Start">
      ...
      <TabPanel header="Reviews">
        <div class="mt-3 card-body">
          <div class="flex justify-end pb-4">
            <NuxtLink class="your-events-btn" to="pr/addReview">
              <BootstrapIcon name="file-earmark-plus" size="2rem" />&nbsp; Add
            </NuxtLink>
          </div>
...
export default {
  ...
  async mounted() {
    activateSubmenu('h5', 'Permanent');
    await this.init();
  },
  methods: {
    async init() {
      this.long = (await superadminGet(`/superadmin-long-banner/Permanent Visa`)).data.data
      this.short = (await superadminGet(`/superadmin-short-banner/Permanent Visa`)).data.data
      this.reviews = (await superadminGet('/superadmin-reviews/Permanent Visa')).data.data;
      this.offers = (await superadminGet(`/home-offers/Permanent Visa`)).data.data;
    },
    async delLong(i) {
