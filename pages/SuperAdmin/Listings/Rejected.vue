<template>
	...
	<DataTable :value="listing" tableStyle="min-width: 50rem">

		<Column field="title" header="title"></Column>
		<Column header="Image">
			<template #body="slotProps">
				<img :src="slotProps.data.image" class="rounded-[12px] w-[100px] h-[90px] object-cover" />
			</template>
		</Column>
...
export default {
	...
	methods: {
		async init() {
			this.listing = (await superadminPost(`/superadmin-all-listing`, { filter: {'su.status':'rejected'} })).data.data;
		},
		async approve() {
			await superadminPut(`/superadmin-update-listing/${this.id._id}`,
