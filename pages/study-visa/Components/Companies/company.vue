...
export default {
  ...
  methods: {
    ...
    async init() {
      await homeGet(`/home-companies/Study Visa`).then((response) => {
        if (response.status) {
          this.data = response.data.data;
        }
      });
      // this.slug=(await homeGet(`/company-slug/${localStorage.getItem('lo')}/${localStorage.getItem('la')}`)).data.data
    },
