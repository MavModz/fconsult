    ...
    <div v-if="agreementStore && agreementStore.length == 0" class="card bg-white border-0 rounded-10">
      <div class="card-body">
        <div class="text-center">
          ...
          <NuxtLink to="/subadmin/Agreement" class="btn btn-danger py-2 px-4 text-decoration-none text-white mb-4">Add New
            Agreements</NuxtLink>
        </div>
      </div>
    </div>
...
export default {
  ...
  mounted() {
   ...
   methods: {
    ...
    async getListing() {
      await subadminGet(`/view-listing/${this.selectedService}`).then(async (response) => {
    ...
    async selectSuggestion(v) {
      this.loading = true
      await subadminPut(`/select-suggestion/${v}`)
      this.loading = false
      this.init()
    },
    async unselectSuggestion(v) {
      this.loading = true
      await subadminPut(`/unselect-suggestion/${v}`)
      this.loading = false
      this.init()
    },
  }
