    ...
    <div v-if="!loading && totalListing == 0" class="card bg-white border-0 rounded-10">
      <div class="card-body">
        <div class="text-center">
          ...
          <NuxtLink to="/subadmin/Categories" class="btn btn-danger py-2 px-4 text-decoration-none text-white mb-4">Add New
            Listing</NuxtLink>
        </div>
      </div>
    </div>
...
export default {
  ...
  async mounted() {
    await this.init()
    this.services = (await subadminGet(`/company-services`)).data.data.map(i => ({ label: i.service, value: i.service }));
  ...
  methods: {
    ...
    async getListing() {
      ...
      try {
        ...
        const response = await subadminGet(`/view-listing/${this.selectedService}`);
