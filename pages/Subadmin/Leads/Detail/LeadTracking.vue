    ...
    <div v-if="users && users.length == 0" class="card bg-white border-0 rounded-10">
      <div class="card-body">
        <div class="text-center">
          ...
          <NuxtLink to="/subadmin/settings/UserRoleManagement" class="btn btn-danger py-2 px-4 text-decoration-none text-white mb-4">Add Subadmin</NuxtLink>
        </div>
      </div>
    </div>
...
export default {
  ...
  async mounted() {
    this.users = (await subadminGet(`/all-employe`)).data.emp.map((i) => ({ label: i.user_id.name, value: i.user_id._id }));
  },
  methods: {
    async assign(v) {
      this.addModal = false;
      await subadminPut(`/assign-leads/${this.$route.params.id}?OnlyAssign=true`, v).then(() => { });
