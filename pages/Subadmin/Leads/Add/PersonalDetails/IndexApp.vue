...
<template>
        ...
        <div class="form-group correspondence-address col-12 pt-4 pt-md-0">
          ...
          <div class="row pt-4">
            <div class="form-group col-12 col-lg-8 d-lg-flex flex-lg-column pt-4 pt-lg-0">
              ...
              <select v-model="lead.corr_country" class="form-select" @change="getState">
            ...
            <div class="form-group col-12 col-lg-4 d-lg-flex flex-lg-column pt-4 pt-lg-0">
              ...
              <select v-model="lead.corr_state" class="form-select" name="event_type" @change="getCity">
          ...
          <div class="row pt-4">
            <div class="form-group col-12 col-lg-8 d-lg-flex flex-lg-column pb-2">
              <label for="datepicker">City</label>
              <select v-model="lead.corr_city" class="form-select" name="event_type">
                <option v-for="option in allCity" :key="option" :value="option">{{ option }}</option>
              </select>
            </div>
            <div class="form-group col-12 col-lg-4 d-lg-flex flex-lg-column pt-4 pt-lg-0">
              <label for="datepicker">Postal Code</label>
              <input type="number" placeholder="Zip Code" v-model="lead.corr_pincode" class="form-control" maxlength="6"
  ...
  <Dialog v-model:visible="addModal" modal header="Add Family Details" :draggable=false class="w-[92vw] sm:w-[60vw] md:w-[50vw] lg:w-[37vw] xl:w-[30vw] 2xl:w-[25vw]">
    <FormKit type="form" @submit="addFamily">
      ...
      <div class="">
        ...
        <FormKit validation="required" name="phone" id="update-events-drpsem-add" type="tel" placeholder="Phone Number"
  ...
  <Dialog v-model:visible="addRefModal" modal header="Refusal Details" :draggable=false class="w-[92vw] sm:w-[60vw] md:w-[50vw] lg:w-[37vw] xl:w-[30vw] 2xl:w-[25vw]">
...
export default {
  ...
  methods: {
    async getState() {
      this.loading = true
      await subadminGet(`/states/${this.lead.corr_country}`).then(response => {
    ...
    async getCity() {
      this.loading = true
      await subadminGet(`/city/${this.lead.corr_state}`).then(response => {
