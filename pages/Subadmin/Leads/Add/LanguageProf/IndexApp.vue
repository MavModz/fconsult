...
<template>
  ...
  <Dialog v-model:visible="addModal" modal header="Add Language Details" :draggable=false>
    <FormKit type="form" @submit="addLanguages" >
      <div>
        <label class="leads-label-assign-add">Examination Name</label>
        <FormKit validation="required" v-model="selectedLanguage" id="update-events-drpsem-add" class="form-select" placeholder="Select Language"
          name="examination_name" type="select" @change="initField" :options="languages" validation-label="Examination name"/>
      </div>
      <div class="mb-3">
        <label class="leads-label-assign-add">Completion Year</label><br />
        <FormKit validation="required" type="date" id="update-events-drpsem-add" name="completion_year" validation-label="Completion year"/>
      </div>
      <div>
        ...
        <FormKit validation="required" name="overall_scrore" id="update-events-drpsem-add" type="text"
          placeholder="CGPA / Points / Grade"  validation-label="Overall score"/>
      </div>
      <div class="leads-addjb-dtls-mdl">
        <div v-if="currentField" v-for="i in currentField">
          <label class="leads-label-assign-add text-capitalize">{{ i }}</label><br>
          <FormKit validation="required" :name="i" id="update-events-drpsem-add-btw" type="text" :placeholder="i" />
        </div>
...
export default {
  ...
  async mounted() {
    ...
    this.languagesAll = (await subadminGet(`/languages`)).data.languages
