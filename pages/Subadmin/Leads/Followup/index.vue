...
export default {
  ...
  data() {
    return {
      ...
      items: [
        { label: 'Edit Lead', command: (e) => { this.$router.push(`/subadmin/leads/update/${e.data.customer_id._id}`) } },
  ...
  async mounted() {
    ...
    this.lead_status_assign = (await subadminGet(`/leadstatus`)).data.leadstatus.map(i => ({ label: i.statusname, value: i.statusname }));
  ...
  methods: {
    ...
    async addFollowup(d) {
      ...
      if (this.edit_key) {
        ...
        await subadminPatch(`/edit-comment/${this.followup_id}`, x)
          .then(async (r) => {
    ...
    async assignSelectedIDs(v) {
      ...
      await subadminPut(`/assign-multiple-followup/`, v).then(() => { });
    ...
    async applyFilter(i) {
      this.loading = true;
      await subadminPost(`/view-followup`, { filter: i }).then(async (response) => {
    ...
    async applySearch(event) {
      ...
      this.loading = true;
      await subadminPost(`/view-followup`, { search: event.target.value }).then(async (response) => {
        this.customers = this.getCustomers(response.data.data);
    ...
    async init() {
      ...
      try {
        this.loading = true;
        const response = await subadminPost(`/view-followup`);
