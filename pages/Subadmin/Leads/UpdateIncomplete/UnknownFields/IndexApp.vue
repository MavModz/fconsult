...
export default {
  ...
  async mounted() {
    ...
    await subadminGet(`/all-lead-fields`).then(async (response) => {
      this.fields = (response.data.customFields)
    })
  },
  methods: {
    async deleteField(i, index) {
      await subadminDelete(`/delete-unknown-field/${this.$route.params.id}/${i}`).then(async (r) => {
    ...
    async bindData(field, index) {
      ...
      await subadminPut(`/bind-unknown-lead-field/${this.$route.params.id}`, this.lead).then(async (r) => {
        this.lead.unknown_fields.splice(index, 1);
