<template>
    <div class="campaigns-dashboard">
        <div class="campaign-header">
            ...
            <button class="add-campaign-button" @click="addCampaign = true; isEditMode = false" v-if="permission?.campaign?.add">+ Add New
                Campaign</button>
        </div>
        <div class="content">
            ...
            <div class="charts-empty">
                <div v-if="!filteredData || filteredData.length === 0" class="campaign-list">
                    ...
                    <button v-else class="add-campaign-button" @click="addCampaign = true">+ Add New Campaign</button>
                </div>
    ...
    <Sidebar v-model:visible="addCampaign" :header="isEditMode ? 'Edit Campaign' : 'Create Your Campaign'"
        position="right" class="pipeline-custom-compaign-header" :style="{ width: '30rem' }">
                ...
                <div class="form-card-wrapper">
                    <div class="form-card-body">
                        <FormKit class="mt-4" label="Campaign Name" validation="required" name="name" type="text"
                            ...
                            v-model="formData.name"><template #label>
...
export default {
    ...
    methods: {
        async init() {
            await subadminGet(`/campaigns`).then((res) => {
        ...
        async handleEditCampaign(campaign) {
            try {
                const response = await subadminGet(`/campaign/${campaign._id}`);
                if (response.data.status) {
                    const campaignData = response.data.data;
                    this.isEditMode = true;
                    this.editingCampaignId = campaign._id;
                    this.selectedManagers = campaignData.managers;
                    this.selectedUsers = campaignData.managers.map(manager => manager._id);
        ...
        addCampaignSubmit(data) {
            ...
            if (leadData != null) {
                leadData.campaign_name = this.formData.name;
                ...
                if (this.isEditMode) {
                    ...
                    subadminPut(`/campaign/${this.editingCampaignId}`, this.campaignData)
                        .then(() => {
                    ...
                    // Create new campaign
                    subadminPost(`/campaign`, this.campaignData)
                        .then(() => {
                            this.addCampaign = false;
                            this.selectedManagers = [];
                            this.$refs.leadauto.finalData = null;
        ...
        async deleteCampaign(campaign) {
            try {
                const response = await subadminDelete(`/campaign/${campaign._id}`);
                if (response.data.status) {
        ...
        async copyCampaign(campaign) {
            try {
                const response = await subadminPost(`/campaign/${campaign._id}/copy`);
                if (response.data.status) {
                    successAlert('Campaign copied successfully');
        ...
        async fetchLeadGraphRange() {
            try {
                if (this.dateRange && this.dateRange[0] && this.dateRange[1] && this.campaign_Id) {
                    ...
                    const res = await subadminPost('/campaign-lead-funnel-data', {
        ...
        async fetchFunnelData() {
            try {
                const res = await subadminGet('/campaign-funnel-data');
        ...
        async fetchSubStatus() {
            try {
                const res = await subadminGet('/leadstatus-with-counts');
                ...
                if (inProgressStatus) {
                    ...
                    // Handle case where "In Progress" status is not found
                    console.error('In Progress status not found');
                }

                // Process Lost Lead Status
