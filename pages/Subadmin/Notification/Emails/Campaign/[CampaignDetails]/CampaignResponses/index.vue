  ...
  <div v-if="isShow">
    <div class="bg-white border-[2px] border-[#Ededed] mb-[26px] rounded-[12px] py-[32px] px-[20px] ">
      ...
      <div class="flex items-start flex-wrap gap-[20px] justify-between">
        <div class="sm:text-[20px] text-[17px] font-medium sm:font-semibold text-[#0D062D] sm:whitespace-normal sm:overflow-visible sm:text-ellipsis whitespace-nowrap overflow-hidden text-ellipsis">
          {{ campaignData.campaign_name }}<span class="text-[#838383] ">•
            {{ campaignData.sender_email }}</span>
        </div>
        <span
          class="flex items-center gap-2 text-[16px] font-semibold cursor-pointer text-[#ff5757] bg-[#FF57571A] py-[8px] px-[19px] rounded-full" @click="refresh"><i
            class="bi bi-arrow-clockwise"></i>Refresh</span>
      </div>
    </div>
    <div class="bg-white mt-[22px] sm:px-[28px] px-[19px] md:px-[35px] sm:py-[23px] py-[18px] md:py-[30px] border-[2px] border-[#ededed]  rounded-[18px] mb-5">
      <div class="flex flex-wrap gap-3 items-center justify-between">
        ...
        <span
          class="flex items-center gap-2 text-[16px] font-medium py-[8px] px-[12px] sm:px-[19px] text-[#676B6C] bg-black/10 rounded-xl cursor-pointer" @click="downloadPDF">Download
...
<Column header="Opened At">
  <template #body="slotProps">
    ...
    <CampaignFilter v-model="showFilterSidebar" @apply-filters="handleFilter"    ref="filterComponent" campaignDetails=true />
    <div v-show="pdfShow">
...
async handleFilter(filters) {
      ...
      const data = await subadminPost(`/campaign-detail-filter-campaigns/${this.$route.params.CampaignDetails}`, filters)
      if (data.data.status) {
    ...
    async campaignDetails() {
      try {
        const data = await subadminGet(`/email-campaign-list/${this.$route.params.CampaignDetails}`)
