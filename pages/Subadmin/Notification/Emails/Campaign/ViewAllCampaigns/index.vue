...
export default {
  ...
  computed: {
    showTemplateOnly() {
      const { resend, id } = this.$route.query;
...
async getPermission(){
    ...
    getItems(row) {
  const items = [];
//  if (this.permission?.notification?.email?.Campaign?.view) {
    items.push({
      label: 'View',
      command: () => {
        this.$router.push(`/subadmin/notification/emails/campaign/${row._id}/campaigndetail`);
      }
    });
  // }
  // Resend option (with permission)
  if (this.permission?.notification?.email?.email_campaign?.add) {
    items.push({
      label: 'Resend',
      command: async () => {
        if (await askConfirm('Do You Want to resend the Campaign')) {
  ...
  // Delete option (with permission)
  if (this.permission?.notification?.email?.email_campaign?.delete) {
    items.push({
      label: 'Delete',
      command: () => {
        this.deleteCampaign(row._id);
    ...
    items.push({
      ...
      command: () => {
        if (row.campaign_status === "in_progress") {
          ...
          errorAlert("Operation not allowed");
        }
      },
      class: 'cancel-button',
    });
  ...
  return items;
    ...
    async deleteCampaign(i) {
      if (await deleteConfirm('Do You Want to delete this Campaign')) {
        await subadminDelete(`/campaign-delete/${i}`);
        this.campaignData()
    ...
    async cancelCampaign(i) {
      if (await askConfirm('Do You Want to Cancle this Campaign')) {
        await subadminPatch(`/email-campaign/${i}`, { campaign_status: "cancelled" });
        this.campaignData()
    ...
    async handleFilter(filters) {
      // console.log('Applied Filters:', filters);
      // const data = await subadminPost('/campaign-filter-campaigns', filters)
      // this.rows = data.data.data
      // this.showFilterSidebar = false;
       console.log('Applied Filters:', filters);
       const data = await subadminPost('/campaign-filter-campaigns', filters);
  ...
  //   const data = await subadminPost('/campaign-filter-campaigns', {}); // Or default payload
  //   this.rows = data.data.data;
  // } else {
  //   const data = await subadminPost('/campaign-filter-campaigns', filters);
  //   this.rows = data.data.data;
  ...
  // this.showFilterSidebar = false;
    ...
    async resendCampaign(i) {
      if (await askConfirm('Do You Want to Resend this Campaign')) {
        await subadminPost(`/resend-campaign/${i}`,);
        this.campaignData()
