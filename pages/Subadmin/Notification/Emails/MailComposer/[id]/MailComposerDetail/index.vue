<template>
  <div class="flex justify-content-end mb-3" v-if="permission?.notification?.email?.email_composer?.add">
    <div class="relative inline-block">
      <router-link to="/subadmin/notification/emails/mailcomposer/composeemail" class="add-button">+ Compose Email
      </router-link>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async detailData() {
      const data = await subadminGet(`/email-composer-detail/${this.$route.params.id}`)
