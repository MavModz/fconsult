<template>
  ...
  <div v-if="isTemplate && permission?.notification?.email?.email_campaign?.add" class="relative inline-block template-block flex justify-content-end mb-3">
 ...
 async setup() {
  ...
  methods: {
    ...
    async submitCampaign(payload) {
      ...
      let data;
      if (this.sourceType === 'html-editor') {
        data = await subadminPost("/email-campaign-html-editor", payloadData)
      } else {
        data = await subadminPost("/email-campaign", payloadData)
      ...
      if (data.data.status) {
        ...
        this.$router.replace({ path: '/subadmin/Notification/Emails/Campaign/ViewAllCampaigns', query: {} }).then(() => {
    ...
    async handleContactList(id) {
      ...
      const data = await subadminGet(`/view-contact-list-basedOn-group/${id}`)
