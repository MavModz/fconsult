<template>
  <div class="wa-campaign-wrapper">
    <div class="wa-campaign-search-header">
      ...
      <div class="new-campaign-wrapper" v-if="permission?.notification?.whatsapp?.campaign?.add">
        <button class="wa-campaign-btn" @click="$router.push('/subadmin/notification/whatsapp/campaigns/broadcast')"><i class="bi bi-plus-circle-fill"></i> Launch New Campaign</button>
      </div>
    </div>
    ...
    <div class="wa-campaign-list">
      ...
      <DataTable :value="filteredCampaignData">
        ...
        <Column header="Action" v-if="permission?.notification?.whatsapp?.campaign?.edit">
          <template #body="slotProps">
            <i class="bi bi-bar-chart" style="cursor:pointer; font-size: 20px;" @click="onEdit(slotProps.data)" v-if="permission?.notification?.whatsapp?.campaign?.edit"></i>
          </template>
        </Column>
      </DataTable>
    </div>
...
this.getPermission();
  ...
  methods: {
    ...
    async fetchWhatsappCampaigns() {
      try {
        const response = await subadminGet('/whatsapp-campaigns');
