<template>
    <div class="message-template-wrapper">
        <div class="add-message-template-container" v-if="permission?.notification?.whatsapp?.my_template?.add">
            <button @click="$router.push('/subadmin/notification/whatsapp/templates/create')"><i
        ...
        <div class="message-template-info-wrapper ">
            ...
            <DataTable :value="filteredTemplates" >
                ...
                <Column header="Action">
                    <template #body="slotProps">
                        <i class="bi bi-pencil-square" style="cursor:pointer; margin-right:10px;"
                            @click="onEdit(slotProps.data)" v-if="permission?.notification?.whatsapp?.my_template?.edit"></i>
                        ...
                        <i class="bi bi-trash" style="cursor:pointer; color:#FF5757;"
                            @click="onDelete(slotProps.data)" v-if="permission?.notification?.whatsapp?.my_template?.delete"></i>
                    </template>
                </Column>
            </DataTable>
        </div>
    ...
    methods: {
        ...
        onEdit(row) {
            if (this.wabaData?.long_token) {
                ...
                this.$router.push({
                    path: '/subadmin/notification/whatsapp/templates/create',
        ...
        async onDelete(row) {
            try {
                const response = await fetch(`https://graph.facebook.com/v20.0/${this.wabaData?.waba_id}/message_templates?hsm_id=${row.id}&name=${row.name}`, {
                    method: 'DELETE',
                    headers: {
                        Authorization: `Bearer ${this.wabaData?.long_token}`,
                    },
