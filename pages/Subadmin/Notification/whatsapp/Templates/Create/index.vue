        ...
        <div class="create-template-form-wrapper">
            ...
            <div class="template-container">
                <div class="template-detail-info">
                    ...
                    <div class="create-template-type">
                    ...
                    <div class="create-template-header">
                        <label for="name">Template Header Text (Optional)</label>
                        <p>Header text is optional and only upto 60 characters are allowed. Use {{1}} for a single variable.</p>
                        <input type="text" name="header" placeholder="Enter Template Header" v-model="header" maxlength="60">
                    </div>
                    ...
                    <div v-if="hasHeaderPlaceholders && type === 'text'" class="create-template-header-examples">
                        <label>Header Example Value</label>
                        <p>
                            Provide a sample value for the header variable so the template can be reviewed. This will be
                            sent as <code>example.header_text</code> in the request. Only one variable ({{1}}) is allowed in header.
                        </p>
                        <div class="template-interaction-field">
                            <label for="header-example-input-1">Variable 1</label>
                            <input id="header-example-input-1" type="text"
                                placeholder="Enter example for {{1}}"
                                v-model="headerExampleValues[0]" maxlength="256" />
                        </div>
                    </div>
                    <div class="create-template-format">
                        <label for="name">Template Format</label>
                        <p>Use text formatting - *bold* , _italic_ & ~strikethrough~Your message content. Upto 1024
                            characters are allowed.e.g. - Hello {{ '{' }}1{{ '}' }}, your code will expire in {{ '{'
                            }}2{{ '}' }} mins.</p>
                    ...
                    <div v-if="hasBodyPlaceholders" class="create-template-body-examples">
                        <label>Body Example Values</label>
                        <p>
                        ...
                        <div class="template-interaction-field" v-for="placeholderIndex in bodyPlaceholderMaxIndex"
                            ...
                            <label :for="'body-example-input-' + placeholderIndex">Variable {{ placeholderIndex }}</label>
            ...
            <div class="template-interaction-wrapper flex flex-col gap-3 mb-10">
                ...
                <div class="template-interaction-field-wrapper">
                    ...
                    <div v-if="actionType === 'reply'">
                        <div class="template-interaction-field" v-for="(reply, index) in quickReplies"
                            :key="'qr-' + index">
                            <label :for="'quick-reply-' + index">Quick Reply {{ index + 1 }}</label>
                    ...
                    <div v-if="actionType === 'call_to_action'">
                        <div class="template-interaction-field" v-for="(cta, index) in ctas" :key="'cta-' + index">
                            <label :for="'cta-' + index">Call to Action {{ index + 1 }}</label>
                    ...
                    <div v-if="actionType === 'all'" class="all-actions-wrapper">
                        ...
                        <div class="action-group">
                            ...
                            <div class="template-interaction-field" v-for="(reply, index) in quickReplies"
                                :key="'all-qr-' + index">
                                <label :for="'all-quick-reply-' + index">Reply {{ index + 1 }}</label>
                        ...
                        <div class="action-group">
                            ...
                            <template v-for="(cta, index) in ctas" :key="'all-url-' + index">
                                <div v-if="cta.ctaType === 'url'" class="template-interaction-field">
                                    <label :for="'all-url-' + index">URL {{ urlCtas.indexOf(cta) + 1 }}</label>
                        ...
                        <div class="action-group">
                            ...
                            <template v-for="(cta, index) in ctas" :key="'all-call-' + index">
                                <div v-if="cta.ctaType === 'call'" class="template-interaction-field">
                                    <label :for="'all-call-' + index">Phone {{ phoneCtas.indexOf(cta) + 1 }}</label>
                                    <input type="text" placeholder="Button Title" v-model="cta.ctaButtonText"
                                        maxlength="25" />
            ...
            <div class="template-save-container" :disabled="isLoading">
                <button @click="submitTemplate"><span v-if="isLoading" class="spinner"
                        style="margin-right:8px;"></span>Submit</button>
            </div>
        </div>
    </div>
</template>
    ...
    methods: {
        ...
        async fetchTemplateData(id) {
            ...
            if (!token) {
                ...
                this.$router.push('/subadmin/notification/whatsapp/templates');
                return;
            }
            try {
        ...
        async submitTemplate() {
            ...
            try {
                ...
                if (!this.category || !this.language || !this.name || !this.type || !this.format) {
                ...
                if (this.isEditMode && this.editId) {
                    ...
                    const response = await subadminPost('/create-message-template', payload);
