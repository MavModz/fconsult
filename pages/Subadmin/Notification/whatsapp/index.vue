			...
			<div class="wa-company-head-right">
				...
				<div class="wa-company-btn-stats flex-wrap">
					<p><span
						...
						Business API</p>
					<p v-if="wabaData"><span>Remaining Quota <i class="bi bi-info-circle-fill"></i></span> 1000</p>
				</div>
			</div>
		</div>
		<div class="wa-business-details">
			<div class="wa-business-details-wrapper">
				<div v-if="wabaData?.profile_status" class="wa-business-details-container">
					<div class="wa-messaging-tabs">
						<div class="waba-messaging-tabs-container cursor-pointer" @click="$router.push('/subadmin/notification/whatsapp/chat')">
						...
						<div class="waba-messaging-tabs-container cursor-pointer" v-if="permission?.notification?.whatsapp?.broadcast?.view" @click="$router.push('/subadmin/notification/whatsapp/campaigns/broadcast')">
							<NuxtImg src="/img/svg/broadcast.svg"  class="tab-image" alt="broadcast-img" />
							<p>Broadcast</p>
						</div>
						<div class="waba-messaging-tabs-container cursor-pointer" v-if="permission?.notification?.whatsapp?.campaign?.view" @click="$router.push('/subadmin/notification/whatsapp/campaigns')">
							<NuxtImg src="/img/svg/campaign.svg"  class="tab-image" alt="campaign-img" />
							<p>Campaign</p>
						</div>
						<div class="waba-messaging-tabs-container cursor-pointer" v-if="permission?.notification?.whatsapp?.flow?.view" @click="$router.push('/subadmin/notification/whatsapp/flow')">
						...
						<div class="waba-business-name-wrapper">
							...
							<div class="waba-account-stats-container">
								<div class="waba-account-stats-tab">
									...
									<div class="waba-account-stats-data">248 +</div>
								</div>
								<div class="waba-account-stats-tab">
									<div class="waba-account-stats-title">Free Tier Conversations <i
											class="bi bi-info-circle-fill"></i></div>
									<div class="waba-account-stats-data">1,200 +</div>
								</div>
								<div class="waba-account-stats-tab">
									<div class="waba-account-stats-title">Quality Ratings <i
											class="bi bi-info-circle-fill"></i></div>
									<div class="waba-account-stats-data">100</div>
								</div>
								<div class="waba-account-stats-tab">
									<div class="waba-account-stats-title">Approcimate Charges <i
											class="bi bi-info-circle-fill"></i></div>
									<div class="waba-account-stats-data">3,488.39</div>
								</div>
							</div>
							<hr class="divider-wrapper" />
			...
			<div class="wa-business-msg mb-4">
				<div :class="['waba-conversation-wrapper', wabaData?.profile_status === 'ACTIVE' ? '' : 'pending']">
					<div class="waba-wcc-header">
						<div class="wb-credits">WhatsApp Conversation Credits (WCC)<i class="bi bi-info-circle-fill ml-2"
								title="WhatsApp Conversation Credits"></i></div>
						<h5>₹ 3,488.39</h5>
					</div>
					<ProgressBar :value="50" :show-value="false"></ProgressBar>
					<div class="waba-cc-balance-wrapper flex-wrap gap-2">
						<span>1248+ Messages Left</span>
						<button><i class="bi bi-plus-circle-fill"></i>Add Balance</button>
					</div>
					<hr class="divider-wrapper" />
					<div class="waba-wcc-footer">
						<span>Free Service Conversation<i class="bi bi-info-circle-fill ml-2"></i></span>
				...
				<div class="waba-profile-detail-wrapper" v-if="wabaData">
					<div class="profile-details-section">
						<h6 class="edit-section-title" @click="editProfileModal = true" v-if="permission?.business_profile?.edit">Edit Profile <i
						...
						<div class="profile-info">
							<div class="info-item-name" v-if="whatsappBusinessPofileData">
								<NuxtImg
									...
									alt="Profile Image"  class="rounded-full w-[60px] h-[60px] xl:w-[90px] xl:h-[90px]" />
							...
							<div class="info-item-website" v-for="(site, index) in whatsappBusinessPofileData?.websites"
								...
								<p class="mb-0"><a :href="`http://${site}`">{{ site }}</a></p>
							</div>
						</div>
					</div>
				</div>
				<div
					...
					<div class="my-template-cta cursor-pointer flex items-center justify-between py-1"
						@click="$router.push('/subadmin/notification/whatsapp/templates')" v-if="permission?.notification?.whatsapp?.my_template?.view">
						<div class="template-cta-container"><i class="bi bi-columns-gap"></i> My Templates</div>
						<i class="bi bi-arrow-up-right"></i>
					</div>
					<hr class="divider-wrapper" v-if="permission?.notification?.whatsapp?.my_template?.view"/>
		...
		async saveWhatsAppData() {
			try {
				if (this.whatsappData) {
					// Send data to your backend
					await subadminPost('/whatsapp-login', { ...this.whatsappData });
