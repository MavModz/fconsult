...
export default {
  ...
  methods: {
    async init() {
      this.loading = true;
      await subadminGet(`/all-customer-invoice/${this.id}`).then(
        async (response) => {
    ...
    async deleteInvoice(i) {
      if (await deleteConfirm('Do You Want to delete this Invoice')) {
        await subadminDelete(`/delete-invoice/${i}`);
        location.reload()
    ...
    addInvoice(id) {
      if (!this.loading) {
        ...
        if (this.allSetup) {
          this.$router.push(`/subadmin/payments/${this.$route.params.id}/new`)
        } else {
          this.infoModal = true
        }
      }
    ...
    try {
      const response = await subadminGet(`/user-invoice-paid/${i}/${emiIndex}`);
    ...
    async markAsPaidFull(i ) {
      await subadminGet(`/user-invoice-paid/${i}`).then(async (res) => {
