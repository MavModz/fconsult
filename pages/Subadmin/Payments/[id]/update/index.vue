...
export default {
  ...
  async setup() {
    let nuxta = (await subadminGet(`/business-profile`)).data.data
    return { nuxta }
  },
  ...
  methods: {
    ...
    async finalSubmit(i) {
      this.loading = true
      await subadminPut(`/update-invoice/${this.$route.params.id}`, this.invoice).then(async (response) => {
        this.$router.go(-1)
      })
      this.loading = false
    },
    async loadInvoiceData() {
      this.loading = true;
      await subadminGet(`/view-invoice/${this.id}`).then(async (response) => {
