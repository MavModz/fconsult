...
<template>
  <div class="lead-setting flex justify-between">
    <h2>Services</h2>
    <p v-if="invoice.total_amount>0"> <span class="text-[#000000] text-medium text-[20px]">Total Cost :</span> <span class="text-[#ff5757]">{{ invoice.total_amount }}</span></p>
  </div>
  <div class="lead-detail row px-5 pb-5">
    <div class="row align-items-center">
      ...
      <div class="form-group col-12 col-md-3 d-md-flex flex-md-column pt-4 pt-md-0">
        ...
        <input type="text" v-model="invoice.service_cost" inputmode="numeric" @input="handleMainCostInput">
      </div>
    ...
    <div class="row" v-for="(row, index) in invoice.additionalRows" :key="index">
      ...
      <div class="form-group col-12 col-md-4 d-md-flex flex-md-column pt-4 pt-md-0">
        <label>Cost</label>
        <input type="text" v-model="row.cost" inputmode="numeric" @input="handleDynamicCostInput(row)" />
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async getListing() {
      this.loading = true
      await subadminGet(`/view-listing/${this.invoice.service}`).then(async (response) => {
