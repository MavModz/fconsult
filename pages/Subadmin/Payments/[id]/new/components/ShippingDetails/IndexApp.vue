...
export default {
  ...
  methods: {
    ...
    async getState() {
      await subadminGet(`/states/${this.invoice.shipping.country}`).then(response => {
        this.allState = response.data
      })
    },
    async getCity() {
      await subadminGet(`/city/${this.invoice.shipping.state}`).then(response => {
    ...
    async searchCustomers() {
      ...
      this.timeoutId = setTimeout(async () => {
        try {
          const response = await subadminGet(
