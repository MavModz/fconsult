<template>
    ...
    <div class="lead-top-section d-flex justify-content-between items-center">
        ...
        <div class="edit-section d-flex gap-2 items-center cursor-pointer" v-if="permission?.customer?.edit"
        ...
        <div>
            ...
            <div class="lead-tabs">
                ...
                <div class="tab d-flex align-items-center gap-[8px]" :class="{ active: activeTab === 'documents' }"
                    @click="handleEmail('documents')" v-if="permission?.customer?.documents?.view"><i class="bi bi-file-earmark-text"></i>
                    Documentation</div>
                <div class="tab d-flex align-items-center gap-[8px]" :class="{ active: activeTab === 'recommendation' }"
                    @click="handleEmail('recommendation')" v-if="permission?.customer?.recommendation?.view"><i class="bi bi-stars"></i>Recommendation</div>
                <div class="tab d-flex align-items-center gap-[8px]" :class="{ active: activeTab === 'appointment' }"
                    @click="handleEmail('appointment')" v-if="permission?.customer?.appointment?.view"><i class="bi bi-calendar-month"></i> Appointments
                </div>
                <div class="tab d-flex align-items-center gap-[8px]" :class="{ active: activeTab === 'invoice' }"
                    @click="handleEmail('invoice')" v-if="permission?.customer?.invoice?.view"> <i class="bi bi-clipboard-minus"></i> Estimate &
                    Invoice</div>
                <div class="tab d-flex align-items-center gap-[8px]" :class="{ active: activeTab === 'agreement' }"
                    @click="handleEmail('agreement')" v-if="permission?.customer?.agreements?.view"> <i class="bi bi-blockquote-left"></i> Agreement
                </div>
                <div class="tab d-flex align-items-center gap-[8px]" :class="{ active: activeTab === 'comments' }"
                    @click="handleEmail('comments')" v-if="permission?.customer?.comments?.view"> <i class="bi bi-card-list"></i>Comments</div>
                <!-- <div class="tab d-flex align-items-center gap-[8px]" :class="{ active: activeTab === 'documents' }"
            ...
            <div class="card-section">
                <div class="dropdown-box rounded-tr-lg rounded-tl-lg">
                    ...
                    <div class="relative w-full">
                        ...
                        <ul v-if="dropdownOpen && permission?.customer?.edit"
                ...
                <div class="dropdown-box">
                    <p class="label d-flex gap-[6px] align-items-center"><img src="public/img/png/campaign-vector.png">
                        Listing</p>
                    <div class="select-option-1" v-if="permission?.customer?.edit">
 ...
 {{ lead?.listing?.title }}
    ...
    <!-- quick-action-button -->
    <div class="conatiner-open" v-if="permission?.customer?.edit">
    ...
    <Sidebar v-model:visible="addSidebar" position="right"
        ...
        title="CSV Upload Status" v-if="permission?.customer?.edit">
    ...
    <Dialog v-model:visible="addFollowup" modal header="Add Follow Up" :draggable="false"
        class="w-[92vw] sm:w-[80vw] md:w-[60vw] lg:w-[40vw] xl:w-[32vw]" v-if="permission?.customer?.edit">
      ...
      try {
        ...
        async initIds() {
            ...
            if (this.$route.query.currintid && this.$route.query.page) {
                await subadminGet(`/circular-customers/${this.$route.query.currintid}/customer`).then(response => {
                ...
                await subadminGet(`/circular-customers/${this.$route.params.id}/customer`).then(response => {
                    this.ids = response.data.data
                    console.log("ids", this.ids)
                    this.currentPage = 1
                    this.totalPages = this.ids.length
