<template>
  <div class="r-container mt-[32px]" v-if="table">
    <div class="top-bar  flex-wrap">
      ...
      <button class="addDocumnetButton" @click="showRecommendationDialog = true" v-if="permission?.customer?.recommendation?.add">+ Add Recommendation</button>
    </div>
    <div class="cards " v-if="activeTab === 'recommend'">
      <div v-if="recommendations.length > 0" class=" recommend-table rounded-xl overflow-hidden">
        <DataTable class=" mt-3  recommend-t leadabc gallery-container  overflow-hidden " :value="recommendations" 
          ...
          <Column field="status" header="Status" style="max-width: 21rem;" class="text-center table-stat">
            <template #body="{ data }">
              ...
              <span v-else class="recmd-tbl-sttss"><p>Unselected</p>
              </span>
            </template>
          </Column>
...
export default {
  ...
  if (this.permission?.customer?.edit) {
    items.push({
      label: 'Comment',
      command: () => {
  ...
  if (this.permission?.customer?.delete) {
    items.push({
      label: 'Delete',
      command: () => {
