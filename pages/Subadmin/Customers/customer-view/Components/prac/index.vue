<template>
  <div class="relative w-64">
    ...
    <!-- Dropdown -->
    <div v-if="dropdownOpen && permission?.customer?.edit" class="absolute border bg-white mt-1 w-full z-10 max-h-60 overflow-auto" >
      <!-- Status list -->
...
async function loadStatuses() {
  try {
    const res = await subadminGet('/customerStage')
...
async function updateStatus(status, subStage) {
  try {
    ...
    if (status === "Converted Lead") {
      ...
      await subadminPut(`/update-customer-stage/${route.params.id}`, {
        lead_stage: status,
...
async function convertCustomer(v) {
  loading.value = true
  try {
    await subadminPut(`/assign-leads/${route.params.id}`, v)
