<template>
    <div class="m-container mt-[32px]">
            ...
            <button class="add-agg" @click="SelectAgreement = true" v-if="permission?.customer?.agreements?.add">Add Agreement</button>
            </div>
            <div class="card-grid mt-[27px]" v-if="filteredAgreements.length">
    ...
    <Dialog v-model:visible="Modal" modal header="Agreement" :style="{ 'width': '50% !important' }" :draggable="false" >
     ...
     <Dialog v-model:visible="SelectAgreement" modal   :draggable="false"  :closable="false"  class="w-[92vw] sm:w-[80vw] md:w-[60vw] lg:w-[45vw] xl:w-[40vw]">
       <template #header>
    <div style="display: flex; justify-content: flex-end; width: 100%; padding: 0.25rem 0.5rem;">
      <Button icon="pi pi-times" class="p-button-rounded h-[16px] p-button-text" @click="SelectAgreement = false" />
        ...
        <FormKit @submit="addRecommendation" type="form" class="leads-form-style" ref="myForm">
                <div class="form-inpt-head mx-3 pb-3">
                    <div class="form-url-frm">
                        <span class="frm-url-wid ">
                            <div class="mt-3 mb-2">
                                <FormKit type="select" :options="agreementStore" name="a_id" id="form-url-inpt"
                                    class="form-select required " placeholder="Select Agreement" label="Add Agreements" validation="required"
            />
                            </div>
                        </span>
                    </div>
...
export default {
    ...
    methods: {
        ...
        async init() {
            this.loading = true
            this.agreementStore = (await subadminGet(`/view-agreement`)).data.data.map(i => ({ label: getSlicedContent(i.document_title, 30), value: i._id }));
            this.allAgreements = (await subadminGet(`/agreements/${this.$route.params.id}`)).data.data
            this.loading = false
        },
        ...
        async addRecommendation(v) {
            this.loading = true
            await subadminPost(`/send-agreement/${v.a_id}/${this.$route.params.id}`).then((r) => {
                this.init()
            }).catch((e) => { });
