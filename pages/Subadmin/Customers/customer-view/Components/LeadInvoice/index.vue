<template>
    <div class="a-container  mt-[32px]">
        <div class="top-bar">
            ...
            <button class="shortlist-btn" @click="$router.push(`/subadmin/payments/${this.$route.params.id}/new`)" v-if="permission?.customer?.invoice?.add">+
...
export default {
    ...
    methods: {
        ...
        async init() {
            try {
                const response = await subadminGet(`/admin-invoice-list-lead/${this.$route.params.id}`);
