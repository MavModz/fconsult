    ...
    <div class="leads-table-filtr-wrapper">
      ...
      <div class="leads-table-tble-chckbxfltr style-4">
        ...
        <div class="leads-table-filter-flx-new p-[10px]">
          <MultiSelect display="chip" v-model="filter.stage" :options="stage" optionLabel="value"
...
export default {
  ...
  methods: {
    submit() {
      ...
      if (this.filter.stage.length > 0) {
        f.lead_stage = { $in: [] }
        for (let i in this.filter.stage) {
          f.lead_stage.$in.push(this.filter.stage[i].value)
        }
      }
      if (this.filter.source.length > 0) {
        f.source = { $in: [] }
        for (let i in this.filter.source) {
          f.source.$in.push(this.filter.source[i].name)
        }
      }
