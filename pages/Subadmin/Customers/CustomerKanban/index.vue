<template>
  <div class="pipline-view-body-container h-screen flex flex-col overflow-hidden bg-[#f9fafb]">
        ...
        <div v-for="(column, index) in allstore" :key="index"
          ...
          <div class="mb-4">
            <div class="rounded-full px-2 md:px-4 py-2 flex items-center justify-between text-white shadow-md"
              ...
              <i v-if="store[index].stage != 'New Client' && permission?.customer?.edit"
          ...
          <div class="flex-1 p-1 md:p-2 bg-white rounded-xl  overflow-y-auto"   @dragover.prevent="permission?.customer?.edit && $event.preventDefault()"
     @drop="permission?.customer?.edit && drop(index)">
           ...
           :draggable="permission?.customer?.edit"
       @dragstart="permission?.customer?.edit && dragStart(card, index, cardIndex)">
              <div class="flex justify-between items-start mb-3">
                <div class="flex items-center gap-3">
                  <img :src="card.image" alt="Avatar" class="w-[52px] h-[52px] rounded-full" />
                  <div>
                    <div class="flex items-center gap-2">
                      <i @click="$router.push(`/subadmin/customers/customer-view/customer-View/${card?._id}`)"
                ...
                <div class="relative">
                  <i class="bi bi-list text-2xl text-[#909090] hover:text-gray-600 cursor-pointer"
                    @click="toggleDropdown(card._id)" v-if="permission?.customer?.edit || permission?.customer?.delete"></i>
                  <!-- Dropdown Menu -->
                  <div v-if="dropdownCardId === card._id"
                    ...
                    <ul class="text-sm text-[#282823] py-2 pl-0 mb-0">
                      <li @click="counsellorModal = true; id = card._id"
                        class="px-3 py-2 hover:bg-gray-100 flex items-center gap-2 cursor-pointer" v-if="permission?.customer?.edit">
                      ...
                      <li class="px-3 py-2 hover:bg-gray-100 flex items-center gap-2 text-[#ff5757] cursor-pointer"
                        @click="showConvertDialog = true; id = card._id" v-if="permission?.customer?.delete">
    ...
    card.convert_customer_date? new Date(card.convert_customer_date)
          .toLocaleString('en-GB', { day: '2-digit', month: '2-digit',
...
export default {
  ...
  methods: {
    ...
    async submitDrop() {
      ...
      await subadminPut(`/update-customer-stage/${card._id}`, {
    ...
    async getSingle(search = '', filters = {}) {
      ...
      const res = await subadminGet(`/customer-stages-with-customers?${query.toString()}`)
  ...
  if (this.colorValue && this.subcategories.length > 0 && this.stageName.trim() !== '') {
    ...
    try {
      ...
      const response = await subadminPost(`/customerStage-add`, payload);
  ...
  if (this.colorValue && this.subcategories.length > 0 && this.stageName.trim() !== '') {
    ...
    try {
      ...
      const res = await subadminPut(`/customerStage-update/${this.id}`, payload);
      if (res.status) {
  ...
  this.AddStage = true;
    ...
    async convertCustomer(v) {
      await subadminPut(`/assign-leads/${this.id}?OnlyAssign=true`, v).then(async (response) => {
