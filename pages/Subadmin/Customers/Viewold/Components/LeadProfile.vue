<template>
  ...
  <Sidebar v-if="data" v-model:visible="offCanvas" header="Customer Profile" position="right"
    style="width: 40vw;overflow: auto;padding: 10px;" class="lead-canvas">
      ...
      <div class="single-lead-profile">
        ...
        <div class="single-lead-inside-name-view">
          <span class="single-lead-actmae">
            <p>{{ getSlicedContent(data.name, 50) }}</p>
          </span>
          <!-- <span>
        ...
        <div class="card">
          ...
          <div class="single-leads-teck-container">
            <div class="mb-3">
              ...
              <FormKit v-model="data.lead_status" id="update-events-drpsemi-drop" class="form-select" type="select"
                name="lead_status" :options="leadstatus" @change="changeStatus()" />
            </div>
            <div class="mb-3">
              <label class="leads-label-assign-ovrvw">Pipeline</label>
              <FormKit v-model="data.pipeline" id="update-events-drpsemi-drop" class="form-select" type="select"
                :options="pipeline" @change="changePipeline()" />
            </div>
            <!-- <div class="mb-3">
            ...
            <span class="single-lead-ovre-cmprfle">
              <p>
                <NuxtLink :to="`/subadmin/leads/detail/${data._id} `">View Complete Profile</NuxtLink>
              </p>
            </span>
            <!-- <span>
...
export default {
  ...
  methods: {
    async getSubCouncellor() {
      this.loading = true
      this.items = (await subadminGet(`/select-employes-list/?role=counsellor`)).data.employe_list.map(i => ({ label: i.name, value: i._id }));
      this.loading = false
    },
    openCanvas(i) {
      this.offCanvas = true
    ...
    async changeStatus() {
      if (await askConfirm('Do You Want to Update The Status', 'Update Status', 'Update', 'No')) {
        this.loading = true
        await subadminPut(`/update-leads-status/${this.data._id}`, { lead_status: this.data.lead_status })
    ...
    async changePipeline() {
      if (await askConfirm('Do You Want to change pipeline', 'Change Pipeline', 'Change', 'No')) {
        this.loading = true
        await subadminPut(`/update-leads/${this.data._id}`, { pipeline: this.data.pipeline })
    ...
    async convertCustomer(v) {
      ...
      this.loading = true
      await subadminPut(`/assign-leads/${this.data._id}`, v).then(async (response) => {
        await subadminPut(`/convert-customer/${this.data._id}`).then(async (response) => {
