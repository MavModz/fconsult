...
export default defineComponent({
  ...
  async mounted() {
    ...
    let data = (await subadminGet(`/home-view-blogs/${lastSegment}`)).data.data;
    ...
    this.category = (await subadminGet(`/company-allblogcategories`)).data.data.map(i => ({ label: i.category_name, value: i._id }));
  ...
  methods: {
    ...
    async submitForm(v) {
      ...
      await subadminPut(`/company-blogs/${this.$route.params.id}`, v);
