    ...
    methods: {
        ...
        async setpaid(i) {
            ...
            if (await askConfirm(`Are you sure you want to make this payment paid?`)) {
                try {
                    const response = await subadminPut(`/set-status-paid/${i}`);
        ...
        async sendReminder(i) {
            try {
                const response = await subadminGet(`/send-invoice-reminder-due-date/${i._id}/${i.total_amount}/${i.shipping.date}/`);
