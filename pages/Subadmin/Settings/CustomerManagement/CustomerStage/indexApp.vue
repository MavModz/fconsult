<template>
  <div class="bg-white p-[30px] my-[30px] rounded-[27px] languages">
    <div class="flex items-center justify-between">
      <span id="languages-heading">Customer Stage</span>
      <span id="add-btn" class="text-[#ff5757] font-semibold border-[1.5px] border-[#ff5757] px-3 py-[6px] rounded-full"
        @click="AddStage = true" v-if="permission?.setting?.customer_management?.add">+ Add Stage</span>
    </div>
      ...
      <table>
        <thead>
          <tr>
            <th>Stage</th>
            <th>Color</th>
            <th v-if="permission?.setting?.customer_management?.edit || permission?.setting?.customer_management?.delete">Action</th>
          </tr>
        </thead>
        <tbody v-if="loading">
        ...
        <tbody v-else>
          <tr v-for="(i, index) in fields" :key="i">
            ...
            <td>
              <div class="btn-group">
                <span @click="handleUpdate(i)" class="td-btn" v-if="permission?.setting?.customer_management?.edit"><img src="/img/svg/edit.svg" alt="" /></span>
                <span @click="deleteStatus(i._id)" class="td-btn" v-if="permission?.setting?.customer_management?.delete"><img src="/public/img/svg/trash.svg" /></span>
              </div>
            </td>
          </tr>
          <!-- ... other rows ... -->
...
export default {
  ...
  methods: {
    ...
    async handleSubmit(v) {
      if (this.colorValue && this.subcategories.length > 0 && this.stageName !== '') {
        ...
        try {
          ...
          const response = await subadminPost(`/customerStage-add`, v);
    ...
    async init() {
      this.loading = true
      await subadminGet(`/customerStage`).then(async (response) => {
    ...
    async updateForm(v) {
      ...
      if (this.colorValue && this.subcategories.length > 0) {
        try {
          const res = await subadminPut(`/customerStage-update/${this.id}`, v);
          if (res.status) {
            this.AddStage = false
            await this.init();
    ...
    async deleteStatus(v) {
      if (await deleteConfirm('Do You Want to delete this Customer Stage')) {
        ...
        const res = await subadminDelete(`/customerStage-delete/${v}`)
