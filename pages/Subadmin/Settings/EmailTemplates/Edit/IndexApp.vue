...
export default defineComponent({
  ...
  data() {
    ...
    this.loading = true;
    await subadminGet(`/blogs/${this.$route.params.id}`,).then((response) => {
  ...
  methods: {
    async submitForm(v) {
      ...
      this.loading = true;
      await subadminPut(`/company-blogs/${this.$route.params.id}`, v).then((response) => {
        this.$router.push('/subadmin/blogs');
      })
      this.loading = false;
    },
