...
export default {
  ...
  methods: {
    ...
    async submitExecForm() {
      this.addModal = false;
       if (this.selectedExec) {
        ...
        this.$router.push({
          path: '/subadmin/settings/UserRoleManagement/Executives',
    ...
    async getSubAdmin() {
      this.loading = true
      this.items = (await subadminGet(`/select-employes-list/?role=manager`)).data.employe_list.map(i => ({ label: i.name, value: i._id }));
      this.loading = false
    },

    async getSubExecutive() {
      this.loading = true
      this.execItems = (await subadminGet(`/select-employes-list/?role=executive`)).data.employe_list.map(i => ({ label: i.name, value: i._id }));
      this.loading = false
    },
