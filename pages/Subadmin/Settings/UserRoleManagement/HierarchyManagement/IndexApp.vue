...
export default {
  ...
  async mounted() {
    this.data = (await subadminGet(`/hierachy-management`)).data.hierarchy
    this.getSubAdmin()
  },
  methods: {
    async getSubAdmin() {
      this.loading = true
      this.items = (await subadminGet(`/select-employes-list/?role=subadmin`)).data.employe_list.map(i => ({ label: i.name, value: i._id }));
      this.loading = false
    },
    async getSubAdminId() {
    ...
    submitForm() {
      this.$router.push({ path: "/subadmin/hierarchy", query: { name: this.name, id: this.id } })
    }
