    ...
    methods: {
      ...
      try {
        ...
        async submitChecklist() {
            ...
            try {
                if (this.mode === 'add') {
                    await subadminPost(`/checklist-add`, [checklistFormatted]);
                    successAlert('Checklist added successfully');
                } else {
                    await subadminPut(`/checklist-update/${this.currentEditId}`, checklistFormatted);
        ...
        async deleteLanguage(i) {
            if (await deleteConfirm('Do You Want to delete this Checklist')) {
                ...
                const res = await subadminDelete(`/checklist-delete/${i}`);
