...
<template>
  <div class="languages ">
    <div class="btn-container">
      ...
      <div class="custom">
        <span id="add-btn" @click="addModal = true" v-if="permission?.setting?.lead_mangement?.add"><i class="bi bi-plus" style="font-size: 1.25rem;" ></i> Add</span>
    ...
    <section>
      ...
      <DataTable v-else v-model:selection="selectedField" :value="fields" dataKey="id" tableStyle="min-width: 50rem">
        ...
        <Column field="_id" header="Action" v-if="permission?.setting?.lead_mangement?.edit || permission?.setting?.lead_mangement?.delete">
          <template #body="slotProps">
            <div  class="btn-group">
              <span class="td-btn" @click="editItem(slotProps)" v-if="permission?.setting?.lead_mangement?.edit">
                <img src="/img/svg/edit.svg" alt="Edit" />
              </span>
              <span   class="td-btn" @click="deleteItem(slotProps)" v-if="permission?.setting?.lead_mangement?.delete">
...
export default {
  ...
  methods: {
    ...
    async init() {
      ...
      await subadminGet(`/custom-field`).then(async (response) => {
    ...
    async submitForm() {
      ...
      await subadminPost(`/custom-field`, this.data).then((response) => {
        this.init();
      });
    },
    async updateForm() {
    ...
    async deleteItem(i) {
      if (await deleteConfirm('Do You Want to delete this Field')) {
        this.loading = true;
        await subadminDelete(`/custom-field/${i.data._id}/${i.data.field_id}`, this.data);
        this.init();
      }
    },
  },
