              ...
              <div class="row">
                <div class="col-sm">
                  <BootstrapIcon name="eye" class="icon1"
                    @click="this.$router.push(`/subadmin/pipeline/view/${data._id}`)" />
                </div>
                <!-- <div class="col-sm">
...
export default {
  ...
  async mounted() {
    this.loading = true
    await this.init()
    this.lead_statuss = (await subadminGet(`/leadstatus`)).data.leadstatus.map(i => ({ name: i.statusname, code: i._id }));
    this.loading = false
  ...
  methods: {
    ...
    async hidden(index) {
      ...
      await subadminPut(`/pipeline/${pipelineId}`, { is_active: isActive })
      this.init();
      this.loading = false
    },
    ...
    async init() {
      this.data = (await subadminGet(`/pipelines`)).data.data
    },
    ...
    async setDefault(i) {
      if (await askConfirm('Do You Want to Make This Pipeline Default, \n it will set all the leads having pipeline', 'Set Default', 'Set', 'No')) {
        this.loading = true
        await subadminGet(`/admin-set-default-pipeline/${i}`)
    ...
    async deletePip(i) {
      if (await deleteConfirm('Do You Want to delete this Pipeline')) {
        await subadminDelete(`/pipeline/${i}`);
        await this.init();
