        ...
        <div class="update-events-left">
          ...
          <div class="update-event-btn">
            <Button class="update-event-change" type="submit" id="upload-btn" label="Add Event" icon="pi pi-save"
              :loading="loading.p" />
            <NuxtLink to="/subadmin/events"><button class="update-event-goback">Go back</button></NuxtLink>
          </div>
...
export default {
  ...
  async mounted() {
    ...
    this.allServices = (await subadminGet(`/company-services`)).data.data
  ...
  methods: {
    ...
    async getState() {
      await subadminGet(`/states/${this.country}`).then((response) => {
        if (response.status) {
    ...
    async getCity() {
      await subadminGet(`/city/${this.state}`).then((response) => {
        if (response.status) {
    ...
    async getCountry() {
      for (let i in this.allServices) {
        if (this.allServices[i].service == this.cservice) {
          this.allCountry = (await subadminGet(`/service-country/${this.allServices[i].service}`)).data.data.map(i => ({ label: i, value: i }));
        }
      }
    },
    ...
    async submitForm(v) {
      ...
      await subadminPost('/company-events', v)
