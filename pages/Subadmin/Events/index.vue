      ...
      <template #header>
        <div class="Your-events-1stbox mb-3">
          <div class="Your-events-1stbox-search">
            ...
            <div class="your-events-addbtn" v-if="permission?.events?.add">
...
export default {
  ...
  // Show Edit only if permission?.events?.edit
  if (this.permission?.events?.edit) {
    items.push({
      label: "Edit",
  ...
  // Show Delete only if permission?.events?.delete
  if (this.permission?.events?.delete) {
    items.push({
      label: "Delete",
      icon: "pi pi-trash",
  ...
  return `Starts in ${remainingDaysStart} day(s)`;
    ...
    async deleteEvent(i) {
      if (await deleteConfirm('Do you want to delete this event?')) {
        await subadminDelete(`/company-events/${i._id}`);
        this.init();
