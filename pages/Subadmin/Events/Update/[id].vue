...
export default {
  ...
  async setup() {
    let data = ref(null)
    await subadminGet(`/company-events/${useRoute().params.id}`)
      .then(response => {
  ...
  methods: {
    ...
    async getState() {
      await subadminGet(`/states/${this.data.country}`).then((response) => {
        if (response.status) {
    ...
    async getCity() {
      await subadminGet(`/city/${this.data.state}`).then((response) => {
        if (response.status) {
    ...
    async submitForm(v) {
      ...
      this.loading.p = true
      await subadminPut(`/company-events/${this.$route.params.id}`, v)
