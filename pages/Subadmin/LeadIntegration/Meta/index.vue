...
<button
  v-else-if="!loading.fb"
  class="divhove"
  @click="launchFBE"
  :disabled="!permission?.leads?.lead_integration?.add"
  :class="{ 'opacity-50 cursor-not-allowed blur-[1px]': !permission?.leads?.lead_integration?.add }"
>
  <img src="/img/svg/meta.svg" alt="" />
  <span v-if="user">Facebook</span>
  <span v-else>Login To Your Facebook Account</span>
</button>
			<!-- <button v-else-if="!loading.fb" class="divhover" @click="launchFBE" v-if="permission?.leads?.lead_integration?.add">
				<img src="/img/svg/meta.svg" alt="" />
				<span v-if="user">Facebook</span>
				<span v-else >Login To Your Facebook Account </span>
			</button> -->
...
export default {
	...
	methods: {
						...
						FB.api("/me/accounts",
							...
							async (response) => {
								if (response.data && response.data.length) {
									...
									subadminPost('/fb-login', x).then((r) => {
										location.reload()
									}).catch((e) => { })
								} else {
		...
		async subscribeToLeadgen(data, campaign_id = null) {
			await subadminPost(`/fb-subscribe/${this.facebookdata._id}`, { form_id: data.id, form_name: data.name, campaign_id: campaign_id })
			try {
				const response = await fetch(
		...
		async unsubscribeFromLeadgen(data, campaign_id = null) {
			await subadminPost(`/fb-unsubscribe/${this.facebookdata._id}`, { form_id: data.id, campaign_id: campaign_id })
			try {
				const response = await fetch(
					`https://graph.facebook.com/v20.0/${this.page_id}/subscribed_apps`,
					{
		...
		async submitForm(v) {
			...
			if (v.pipeline) {
				await subadminPost(`/fb-lead-import-pipeline/${v.campaign}`, v)
			} else {
				v.pipeline = null
				await subadminPost(`/fb-lead-import`, v)
			}
		...
		async fetchCampaigns() {
			await subadminGet(`/campaigns/is_active`)
				.then(response => {
