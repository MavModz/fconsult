<template>
  <div v-if="data" class="languages webhook-container">
    <div class="btn-container">
      ...
      <Button label="Link New Sheet" class="btn btn-primary" @click="sheetModal = true" v-if="permission?.leads?.lead_integration?.add"/>
    </div>
    ...
    <table>
      ...
      <tbody>
        <tr v-for="i in sheet">
          ...
          <td><Button label="Copy Sheet URL" @click="copyToClipboardKey(i.sheet_url)" class="btn btn-primary" /></td>
        </tr>
      </tbody>
    </table>
    ...
    <FormKit type="form" @submit="findSheet">
      ...
      <div class="code flex flex-col gap-3">
        <FormKit label="Sheet Name" validation="required" type="text" name="sheet_name" placeholder="Google Sheet Name">
          <template #label>Sheet Name <span style="color: red;">*</span></template>
        </FormKit>
        <FormKit label="Google Sheet" validation="required" type="text" name="url" placeholder="Google Sheet Url">
          <template #label>Google Sheet Url <span style="color: red;">*</span></template>
        </FormKit>
        <FormKit label="Select campaign" validation="required" type="select" name="campaign"
          placeholder="Select campaign" :options="allCampaign"><template #label>Selct Campaign <span
...
export default {
  ...
  methods: {
    async init() {
      await subadminGet(`/google-sheet-automation`).then((r) => {
    ...
    async submitForm(v) {
      await subadminPost(`/google-sheet-automation`, v).then((r) => {
        this.init()
      }).catch((e) => { })
    },
    ...
    async findSheet(v) {
      ...
      await subadminPost(`/read-google-sheet`, v).then(async (response) => {
    ...
    async addLead() {
      ...
      for (let i = 0; i < this.leadData.length; i++) {
        ...
        await subadminPost(`/add-leads-google-spreadsheet`, this.leadData[i]).catch((e) => { })
      }
      setTimeout(() => {
