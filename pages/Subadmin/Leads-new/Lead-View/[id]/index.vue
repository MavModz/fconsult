<template>
    ...
    <div class="lead-top-section d-flex justify-content-between items-center">
        ...
        <div class="edit-section d-flex gap-2 items-center cursor-pointer" v-if="permission?.leads?.edit"
            @click="(e) => { this.$router.push(`/subadmin/leads/update/${this.$route.params.id}`) }">
        ...
        <div>
            <div class="lead-header mb-[25px] d-flex align-items-top justify-content-between flex-wrap ">
                ...
                <div class="lead-actions flex">
                    ...
                    <!-- <select class="custom-select cursor-pointer" required>
                            <option>Open Lead / Call Back Later</option>
                        </select> -->
                    <div class="">
                        <prac :status="lead.lead_status" :subStatus="lead.lead_sub_status" @trigger-init="init" />
                    </div>
                    <!-- <select v-model="lead.lead_status" class="custom-select cursor-pointer" required
                            @change="changeStatus">
            ...
            <div class="lead-tabs">
                ...
                <div class="tab d-flex align-items-center gap-[8px]" :class="{ active: activeTab === 'documents' }"
                    @click="handleEmail('documents')" v-if="permission?.leads?.documents?.view"><i class="bi bi-file-earmark-text"></i>
                    Documentation</div>
                <div class="tab d-flex align-items-center gap-[8px]" :class="{ active: activeTab === 'recommendation' }"
                    @click="handleEmail('recommendation')" v-if="permission?.leads?.recommendation?.view"><i class="bi bi-stars"></i>Recommendation</div>
                <div class="tab d-flex align-items-center gap-[8px]" :class="{ active: activeTab === 'appointment' }"
                    @click="handleEmail('appointment')" v-if="permission?.leads?.appointment?.view"><i class="bi bi-calendar-month"></i> Appointments
                </div>
                <div class="tab d-flex align-items-center gap-[8px]" :class="{ active: activeTab === 'invoice' }"
                    @click="handleEmail('invoice')" v-if="permission?.leads?.invoice?.view"> <i class="bi bi-clipboard-minus"></i> Estimate &
                    Invoice</div>
                <div class="tab d-flex align-items-center gap-[8px]" :class="{ active: activeTab === 'agreement' }"
                    @click="handleEmail('agreement')" v-if="permission?.leads?.agreements?.view"> <i class="bi bi-blockquote-left"></i> Agreement
                </div>
                <div class="tab d-flex align-items-center gap-[8px]" :class="{ active: activeTab === 'comments' }"
                    @click="handleEmail('comments')"v-if="permission?.leads?.comments?.view"> <i class="bi bi-card-list"></i>Comments</div>
            </div>
        </div>
    </div>
            ...
            <div class="card-section">
                <div class="dropdown-box rounded-tr-lg rounded-tl-lg">
                    ...
                    <div class="relative w-full">
                        ...
                        <ul v-if="dropdownOpen && permission?.leads?.edit"
                ...
                <div class="dropdown-box">
                    ...
                    <div class="select-option-1" v-if="permission?.leads?.edit">
                    ...
                    <div v-else>{{  lead_campaign_name  }}</div>
                </div>
                <div class="dropdown-box ">
                ...
                <div class="dropdown-box rounded-br-lg rounded-bl-lg">
                    ...
                    <div v-else>
                         {{ lead?.listing?.title ? lead.listing.title : 'Select Status' }}
                        <!-- {{ lead?.listing?.title }} -->
                    </div>
                </div>
            </div>
    ...
    <!-- quick-action-button -->
    <div class="conatiner-open" v-if="permission?.leads?.edit">
    ...
    <Sidebar v-model:visible="addSidebar" position="right"
        class="  h-fit  w-[88vw] sm:w-[55vw]  md:w-[42vw]  lg:w-[32vw]  xl:w-[26vw] 2xl:w-[20vw]  border-t-[4px] border-[#18353B] rounded-lg" v-if="permission?.leads?.edit">
    ...
    <Sidebar v-model:visible="addThirdSidebar" position="right"
        ...
        <div class="call-action-item-1" @click="callStatus = 'not connected', addFollowup = true">
            Add Followup<img src="/public/img/png/side-arrow.png">
        </div>
    </Sidebar>

    <Dialog v-model:visible="addFollowup" modal header="Add Follow Up" :draggable="false"
        class="w-[92vw] sm:w-[80vw] md:w-[60vw] lg:w-[40vw] xl:w-[32vw]" v-if="permission?.leads?.edit">
    ...
    <Dialog v-model:visible="counsellorModal" :header="assign ? 'Assign lead' : 'Convert To Customer'" @update:visible="(val) => {
        counsellorModal = val;
        if (!val) assign = false;
    }" class="w-[92vw] sm:w-[80vw] md:w-[60vw] lg:w-[40vw] xl:w-[32vw]" v-if="permission?.leads?.edit">
    ...
    methods: {
      ...
      try {
        ...
        async markFollowuped() {
            ...
            try {
                subadminPut('/update-followup-status', { ids: this.dueFollowupIds })
        ...
        handleSubmit() {
            this.$router.push({
                path: `/subadmin/leads/update/${this.$route.params.id}`,
            });
        },
        async init() {
            this.lead.loading = true
            await subadminGet(`/leads/${this.$route.params.id}`).then(response => {
        ...
        async initIds() {
            ...
            if (this.$route.query.currintid && this.$route.query.page) {
                await subadminGet(`/circular-customers/${this.$route.query.currintid}/lead`).then(response => {
                ...
                await subadminGet(`/circular-customers/${this.$route.params.id}/lead`).then(response => {
                    this.ids = response.data.data
                    console.log("ids", this.ids)
                    this.currentPage = 1
                    this.totalPages = this.ids.length
        ...
        prevPage() {
            ...
            this.$router.push({
                path: `/subadmin/leads-new/lead-view/${targetId}`,
        ...
        async selectPriority(value) {
            ...
            this.dropdownOpen = false; // Close dropdown immediately
        ...
        async updatePriority() {
            ...
            this.loading = true
            await subadminPut(`/update-leads-status/${this.lead._id}`, { priority: this.selectedPriority });
        ...
        async updateoptService() {
            if (await askConfirm('Do you want to update the service?', 'Update Service', 'Update', 'No')) {
                this.loading = true
                await subadminPut(`/update-leads-status/${this.lead._id}`, { service: this.lead.service })
        ...
        async updateoptCampain() {
            if (await askConfirm('Do you want to update the campaign?', 'Update Campaign', 'Update', 'No')) {
                ...
                await subadminPut(`/update-lead-campaign/${this.lead._id}/${this.lead.campaign}`, { service: this.lead.service })
                successAlert('Lead service updated successfully!');
                this.$parent.init()
            }
        },
        async updateoptListing() {
            if (await askConfirm('Do you want to update the Listing?', 'Update Listing', 'Update', 'No')) {
                ...
                await subadminPut(`/update-leads-status/${this.lead._id}/`, { listing: this.listingId })
        ...
        async handleOtherStatus() {
            if (await askConfirm('Do you want to update the status?', 'Update Status', 'Update', 'No')) {
                this.loading = true
                await subadminPut(`/update-leads-status/${this.lead._id}`, { lead_status: this.lead.lead_status })
        ...
        async getSubCouncellor() {
            this.loading = true
            this.items = (await subadminGet(`/select-employes-list`)).data.employe_list.map(i => ({ label: i.name, value: i._id }));
            this.loading = false
        },
        async convertCustomer(v) {
            this.loading = true
            if (this.convert_lead) {
                await subadminPut(`/assign-leads/${this.lead._id}`, v).then(async (response) => {
                    await subadminPut(`/convert-customer/${this.lead._id}`).then(async (response) => {
                        this.$router.push("/subadmin/customers/view")
                ...
                await subadminPut(`/assign-leads/${this.lead._id}?OnlyAssign=true`, v).then(async (response) => {
        ...
        async listingList() {
            ...
            const Listingdata = await subadminPost('/service-subservice-listing', payload)
        ...
        async initCampaings() {
            ...
            this.loading = true
            await subadminGet(`/campaigns`).then(async (response) => {
                this.campaigns = response.data.data
                if(!this.permission?.leads?.edit){
                    console.log("permission check",this.lead.campaign,this.campaigns.length)
 if(this.lead.campaign!==null && this.campaigns.length>0){
    console.log("lead permission")
  this.lead_campaign_name=this.campaigns.filter((ele,i)=>ele._id==this.lead.campaign)[0].campaign_name
 }
        ...
        async addRemarkComment(v) {
            ...
            this.loading = true;
            if (this.callStatus == 'not connected') {
                v.call_time = '00 : 00 : 00'
            }
            await subadminPost(`/add-comment`, v).then(response => {
            })
            this.init()
