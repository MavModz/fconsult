<template>
  <div class="cards" style="box-shadow: none;">
    <div class="card-title" id="addcmp-dtls-brk">
      ...
      <span class="add-btn hover-effect" @click="$refs.awardModal.addAwardModal = true" v-if="permission?.business_profile?.add">
  ...
  <div class="cards">
    <div class="card-title py-3">
      ...
      <span @click="document_section" class="edit_btn award-add-mdl-stl divhover" v-if="permission?.business_profile?.edit"><img src="/img/svg/dark-edit.svg"
          alt="" />&nbsp;Edit</span>
    </div>
    <div class="card-row">
      ...
      <div class="card-col doc-card" v-if="additional?.length > 0" v-for="(doc, index) in additional" :key="index">
        ...
        <div class="document-name-view">
          ...
          <span @click="deleteAddtional(doc._id)" v-if="permission?.business_profile?.delete"><i class="pi pi-trash"
  ...
  <div class="cards">
    <div class="card-title py-3">
      ...
      <span class="edit_btn divhover award-add-mdl-stl" @click="update_social_link" v-if="permission?.business_profile?.edit"><img src="/img/svg/dark-edit.svg"
          alt="" />&nbsp;Edit</span>
    </div>
...
export default {
  ...
  // Delete option
  if (this.permission?.business_profile?.delete) {
    items.push({
      label: "Delete",
      icon: "pi pi-trash",
      command: () => this.deleteBlog(i),
  ...
  // Edit option
  if (this.permission?.business_profile?.edit) {
    items.push({
      label: "Edit",
      icon: "pi pi-pen-to-square",
      command: () => this.openEditAwardModal(i),
  ...
  return items;
    ...
    async deleteBlog(i) {
      ...
      if (await deleteConfirm("Do You Want to delete this awards?")) {
        await subadminDelete(`/company-awards/${i._id}`)
        this.init();
    ...
    async deleteAddtional(i) {
      ...
      if (await deleteConfirm(`Do you want delete this Addtional document?`)) {
        const res = await subadminDelete(`/company/additional-document/${i}`);
        if (res.status) {
    ...
    async init() {
      await subadminGet(`/company-awards`).then(async (response) => {
