<template>
  <Dialog v-model:visible="addModalDialogue" modal header="Add Video" :style="{'min-width': '380px'}" :draggable="false" @update:visible="clearModalData">
    <section>
      <FormKit type="form" @submit="submitForm" class="mb-0">
        <div class="upload-image-box upload-video">
          ...
          <label class="image-upload-container rounded-2xl mb-3 mx-2 shadow-lg cursor-pointer" data-kt-image-input-action="change"
  ...
  methods: {
    async submitForm(i) {
      if (!await this.checkUrl(i.url)) {
        ...
        i.url = this.videoId
        i.primary=true
        await subadminPost(`/company-videos/`, i)
