    ...
    methods: {
        async changeUser() {
            if (await askConfirm('Do You Want to Change Default Appointment Assignee', 'Channge Assignee', 'Change', 'Cancel')) {
                await subadminPut(`/admin-appointment-assign/${this.defaultUser}`)
                location.reload()
            }
        },
        allWeek() {
        ...
        async init() {
            await subadminGet(`/company-settings`).then((r) => {
            ...
            subadminGet('/admin-google-token').then(async (r) => {
                if (r.data.data) {
                    this.google = true
                    this.googlebtn = true
                }
                    ...
                    callback: async (response) => {
                        if (response.code) {
                            try {
                                ...
                                await subadminPost('/admin-google-token', d.tokens)
                            } catch (error) {
        ...
        async saveappointment() {
            await subadminPut(`/setting-update`, { appointments: this.data }).then((r) => {
                successAlert("Appointment Updated!...")
                this.init()
            }).catch((e) => { })
        },
        async saveappointmentFreq() {
            await subadminPut(`/setting-update`, { appointmentsfreq: this.appointmentsfreq }).then((r) => {
                successAlert("Appointment Updated!...")
                this.init()
            }).catch((e) => { })
        }
