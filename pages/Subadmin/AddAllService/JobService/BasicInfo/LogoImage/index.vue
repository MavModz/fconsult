...
<template>
  <div class="bg-[#F9F8FA] flex justify-center items-center rounded-[50%] relative">
    <!-- <div class="cstcoverimage-serd" v-if="listService.logoImage && listService.logoImage">
      ...
      <div class="cstcoverimage-serd" v-if="listService.logoImage">
    <img class="rounded-full w-full h-full object-cover" :src="listService.logoImage" alt="Logo" />

    <!-- Edit Icon (Pencil) -->
    <label
      class="absolute h-8 w-8 top-1 right-1 bg-white p-1.5 rounded-full shadow cursor-pointer hover:bg-gray-100 transition"
      title="Change Logo"
    >
      <input
        type="file"
    ...
    <input class="opacity-0 absolute h-[-webkit-fill-available] w-[-webkit-fill-available]" type="file"
      @change="handleImageLogo($event)"  accept="image/*" />
  </div>
