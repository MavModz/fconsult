...
<template>
  <div class="mb-2 relative">
    <label class="text-[#1F1B2D] text-[17px]  mb-3" for="">Images that might be helpful in describing your
      job</label>
    <div
      class="mx-auto border-upload text-[#9691A4] text-[25px] outline-none h-[60px] pr-[40px!important] rounded-[11px!important] font-[400] w-[90%!important relative">
      <input ref="fileInput" class="opacity-0 absolute h-[-webkit-fill-available] w-[-webkit-fill-available]"
        type="file" @change="handleImageChange($event)" hidden  accept="image/*"  />
      
      <span class="absolute top-[15px] left-[12px] text-[17px]" v-if="listService.document_file_name">
        {{ listService.document_file_name }}
      </span>
...
export default {
  ...
  methods: {
    handleImageChange(event) {
      ...
      if (!file) return;
      this.listService.document_file_name = file.name;
    ...
    async icon_uploadImage() {
      ...
      this.listService.document_url = uploadedImageUrl;
