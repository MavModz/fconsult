    ...
    <div class="workflow-wrapper">
        ...
        <div class="workflow-info-wrapper">
            <div class="workflow-search-header">
                ...
                <div class="btn-header" v-if="permission?.workFlow?.add">
                    <button @click="$router.push('/subadmin/workflow/create')"><i class="bi bi-plus-circle-fill" ></i>
                        Create New WorkFlow</button>
                </div>
            </div>
            <hr class="divider-wrapper" />
            <DataTable :value="filteredWorkflows" :paginator="true" :rows="10">
                ...
                <Column header="Action">
                    <template #body="slotProps">
                        <i class="bi bi-pencil-square" style="cursor:pointer; margin-right:10px;"
                            @click="onEdit(slotProps.data)" v-if="permission?.workFlow?.edit"></i>
                        ...
                        <i class="bi bi-trash" style="cursor:pointer; color:#FF5757;"
                            @click="onDelete(slotProps.data)" v-if="permission?.workFlow?.delete"></i>
                    </template>
                </Column>
            </DataTable>
        </div>
...
export default {
    ...
    methods: {
        async fetchWorkflows() {
            try {
                const response = await subadminGet('/get-workflows');
        ...
        async confirmDelete() {
            try {
                const response = await subadminDelete(`/delete-workflow/${this.workflowToDelete._id}`);
