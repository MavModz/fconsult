...
export default {
    ...
    methods: {
        async fetchCampaigns() {
            try {
                const response = await subadminGet('/campaigns');
        ...
        async fetchLeadSources() {
            try {
                const response = await subadminGet('/leadsource');
                if (response.status === true) {
        ...
        async fetchStatus() {
            try {
                const response = await subadminGet('/leadstatus');
                if (response.status === true) {
        ...
        async fetchWorkflowDetails() {
            try {
                const response = await subadminGet(`/get-workflow/${this.workflowId}`);
        ...
        async createWorkflow() {
            ...
            try {
                if (this.isEditMode) {
                    // PATCH update
                    const response = await subadminPatch(`/update-workflow/${this.workflowId}`, workflowData);
                    if (response.status === true) {
                        successAlert('Workflow updated successfully');
                        this.$router.push('/subadmin/workflow');
                    } else {
                        errorAlert(response.message || 'Failed to update workflow');
                    }
                } else {
                    // POST create (for both create and copy)
                    const response = await subadminPost('/add-workflow', workflowData);
                    if (response.status === true) {
        ...
        async syncTemplates() {
            this.isSyncing = true;
            try {
                const response = await subadminGet('/sync-message-templates');
                if (response.status === true) {
        ...
        async emailTemplateList() {
            try {
                const data = await subadminGet("/template-name-list")
                if (data.data.status) {
        ...
        async checkPrimaryEmail() {
            if (this.selectedCommunication === 'email') {
                const email = await subadminGet('/email-setting')
