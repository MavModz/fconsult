<template>
  <div class="w-full">
    <div class="md:w-[88%] w-[90%] sm:w-[80%] lg:w-[80%] m-auto">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-3 sm:gap-5 md:gap-8  mt-[50px] sm:mt-[100px]">

        <div v-for="(card, index) in cards" :key="card.title"  @click="toggleCard(card.title)" 
          class="card-category-onboarding group flex items-center gap:[70px] xl:gap-[90px] transition justify-between cursor-pointer">
          <div class="flex gap-2 sm:gap-3">
            <div
              ...
              <img :src="card.hover_icon" alt="hover icon"
                class="icon-hover hidden group-hover:block w-5 h-5 sm:w-6 sm:h-6 object-contain" />
            </div>

            <div>
              <p class="text-lg mb-0 sm:mb-2 md:mb-0 xl:mb-2 font-semibold text-gray-800">{{ card.title }}</p>
      ...
      <div class="mt-[100px]">
        <div class="text-center ">
          <button class="onboarding-step3-subcategory bg-[#ff5757] text-white  px-[60px] sm:px-[120px] py-2 rounded cursor"
    ...
    return {
      ...
      cards: [
        ...
        },
        {
          ...
          selectedSubcategories: ['Tourist Visa','Visitor Visa'],
          countries:[],
        },
      ],
    };
