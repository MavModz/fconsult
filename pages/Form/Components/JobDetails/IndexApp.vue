<template>
  <div class="educational-details-container">
    <div v-if="form.job" class="primary-education-container">
      <div class="course-section">
        <div class="course-heading">
          <span class="fs-4 fw-semibold text-[#808080]" :class="{required: form.job}">Job Details</span>
        </div>
        <button class="course-add-details" @click="addModal = true">+ Add Details
        </button>
      </div>
        <span v-if="errors.job" class="text-red-500">{{ errors.job }}</span>
      <div class="education-course-details-pts" v-if="lead.job && lead.job.length > 0">
  ...
  <Dialog v-model:visible="addModal" modal header="Job Details">
    <FormKit type="form" @submit="addJob">
      ...
      <div class="leads-addjb-dtls-mdl mt-[18px]">
        <div class="">
          <label class="leads-label-assign-add required">Job Role</label><br>
          <FormKit validation="required" name="Job Role" id="update-events-drpsem-add-btw" type="text"
            placeholder="------" />
        </div>

        <div class="">
          <label class="leads-label-assign-add required">CTC</label><br>
          <FormKit validation="required" name="CTC" id="update-events-drpsem-add" type="text" placeholder="------" />
        </div>
      </div>
      <div class="mt-[18px]">
        <label class="leads-label-assign-add required">Starting Year</label><br />
        <FormKit validation="required" type="date" id="update-events-drpsem-add" name="Starting Year" />
      </div>
      <span>
        <div class="mt-[18px]">
          <label class="leads-label-assign-add required ">Ending Year</label><br />
          <FormKit validation="required" type="date" id="update-events-drpsem-add" name="Ending Year" />
        </div>
      </span>
