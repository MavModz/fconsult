<template>
    ...
    <div v-if="form.formFields.languages" class="primary-education-container">
      <div class="course-section">
        <div class="course-heading">
          <span class="fs-4 fw-semibold text-[#808080]" :class="{required: form.formFields.languages}">Language Proficiency Details</span>
        </div>
        <button class="course-add-details" @click="addModal = true">+ Add Details
        </button>
      </div>
        <span v-if="errors.languages" class="text-red-500">{{ errors.languages }}</span>
      <div v-if="lead.languages && lead.languages.length > 0" class="education-course-details-pts">
  ...
  <Dialog v-model:visible="addModal" modal header="Language Proficiency Details">
    <FormKit type="form" @submit="addLanguages">
      <div class="mt-[15px]">
        <label class="leads-label-assign-add required">Examination Name</label>
        <FormKit validation="required" v-model="selectedLanguage" id="update-events-drpsem-add" class="form-select"
          placeholder="Select Language" name="Examination Name" type="select" @change="initField"
          :options="languages" />
      </div>
      <div class=" mt-[18px]">
        <label class="leads-label-assign-add required">Completion Year</label><br />
        <FormKit validation="required" type="date" id="update-events-drpsem-add" name="Completion Year" />
      </div>
      <div class="mt-[18px]">
        <label class="leads-label-assign-add  required">Overall Score</label><br>
        <FormKit validation="required" name="Overall Scrore" id="update-events-drpsem-add" type="text"
          placeholder="CGPA / Points / Grade" />
      </div>
      <div class="leads-addjb-dtls-mdl mt-[18px]">
        <div v-if="currentField" v-for="i in currentField">
          <label class="leads-label-assign-add required">{{ i }}</label><br>
