  ...
  <div class="offers-container  customclass-listing-container">
    <div class="d-flex flex-wrap justify-content-between gap-2 px-xl-5 px-lg-4 px-md-3 px-sm-2 px-xs-1 py-2">
      ...
      <div class="d-flex align-items-center gap-3">
        <span @click="goToSearch('business-listing', 'Permanent Residency')" class="ms-4 cursor-pointer">View all <i
...
export default {
  ...
  methods: {
    ...
    async fetchData(country = '') {
      try {
        const url = country
          ? `/home-review-companies-all/Permanent Residency/${country}`
          : `/home-review-companies-all/Permanent Residency`;
    ...
    async countriesServices() {
      try {
        const response = await homeGet(
          `/home-review-Listing-country/Permanent Residency`,
        );
        if (response.status) {
