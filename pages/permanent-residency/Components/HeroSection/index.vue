...
export default {
  ...
  async setup() {
    let hero = (await homeGet(`/home-hero-banner/pr`)).data.data
    return { hero };
  },
  mounted() {
    this.trendComp();
  ...
  methods: {
    async trendComp() {
      try {
        let response_trend = await homeGet(`/home-trending-country/Permanent Residency`)

        if (response_trend.status) {
          this.c = response_trend.data.data
        }
      } catch (error) {
