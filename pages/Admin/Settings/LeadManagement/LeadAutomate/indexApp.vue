<template>
    <div class="bg-white pb-[35px] pt-[30px] px-[35px] rounded-[27px] languages">
        <div class="flex items-center justify-between">
            <h2 id="languages-heading">Lead Automation</h2>
            <div class="flex items-center gap-2">
                <span class="text-[17px] font-medium">{{ settings.leadAutomation ? 'On' : 'Off' }}</span>
        ...
        <div class="break-line" v-if="settings.leadAutomation"></div>
...
/>
  ...
  @update:modelValue="() => toggleUserSelection(emp.user_id._id)" />
        ...
        <button @click="submitForm" v-if="settings.leadAutomation" class="bg-[#ff5757] px-[20px] py-[10px] text-[white] mt-[10px] rounded-lg ">
            Save</button>
    </div>
</template>
<script>
export default {
    ...
    methods: {
        ...
        async getSubadmin() {
            const response = await adminGet(`/all-employe`);
        ...
        async submitForm() {
            ...
            if (this.method == "Round_Robin") {
                ...
                try {
                    const response = await adminPost(`/update-lead-settings`, d);
            ...
            if (this.method == "Service_Based") {
                ...
                try {
                    const response = await adminPost(`/update-lead-settings`, data);
                    successAlert("Lead Automation Updated");
                    this.init();
                } catch (err) {
        ...
        async init() {
            const res = await adminGet('/lead-settings');
