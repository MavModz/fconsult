...
<template>
                ...
                <div class="user-invoice-main-first-container">
                    <div class="user-invoice-main-first-container-fees">
                        <div class="user-invoice-main-first-container-fees-first">
                            <p>Consultation Fee&nbsp;: <span>Rs. {{formatIndian (invoice?.total_amount )}}/-</span></p>
                        </div>
  ...
  :href="`/invoice/emifull/${invoice?._id}`" 
  target="_blank" 
  class="flex items-center text-[#676b6c] gap-2"
>
...
export default {
    ...
    methods: {
        getItems(invoice, emi) {
    return [
      {
        label: 'Send Reminder',
        icon: 'pi pi-bell',
        command: () => this.sendReminder(invoice._id, emi.date, emi.amount)
      }
    ];
  },
 formatIndian(amount) {
  ...
  return Math.round(amount).toLocaleString('en-IN');
        ...
        // },
getUnpaidAmount(emiArray) {
  const total = emiArray
    .filter(emi => emi.status === "unpaid") // Only unpaid EMIs
    .reduce((sum, emi) => sum + emi.amount, 0); // Sum amounts

  return this.formatIndian(total); // Apply formatting
},
