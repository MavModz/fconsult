...
export default {
  ...
  async mounted() {
    this.loading = true
    await userGet(`/verify-plan-payment/${this.route.query.razorpay_payment_id}`).then((response) => {
      if (response.status) {
        if (response.data.data.status === "captured") {
          adminRegenerateToken().then((r) => {
            if(response.data.data.notes.type=="app"){
              location.replace('/admin/app-onboarding')
            }
            else if(response.data.data.notes.type=="domain"){
              location.replace('/admin/apps/ondomain/connectdomainfirst')
            } 
            else{
