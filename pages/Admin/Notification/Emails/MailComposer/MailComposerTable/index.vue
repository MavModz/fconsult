  ...
  <div class="bg-white mt-[22px] sm:px-[28px] px-[19px] md:px-[35px] sm:py-[23px] py-[18px] md:py-[30px] mb-5 border-[2px] border-[#ededed]  rounded-[18px]">
    ...
    <DataTable :value="filteredRows" class="rounded-[28px] m-0 border leadabc border-[#E2E8F0]"
      ...
      <Column header="Campaign">
        <template #body="{ data }">
          <router-link :to="`/admin/notification/emails/mailcomposer/${data._id}/mailcomposerdetail`"
...
import ComposeEmail from '.././ComposeEmail/index.vue';
...
export default {
  ...
  methods: {
    async handleFilter(filters) {
      ...
      const data = await adminPost('/composer-filter-campaigns', filters)
      ...
      return [
        {
          label: 'View',
          command: () => {
            this.$router.push(`/admin/notification/emails/mailcomposer/${row._id}/mailcomposerdetail`)
          },
        },
        {
          label: 'Edit',
          command: () => {
            this.$router.push({ path: '/admin/notification/emails/mailcomposer/composeemail', query: { id: row._id } })
          },
        },
        {
          label: 'Resend',
    ...
    async deleteCampaign(i) {
      if (await deleteConfirm('Do You Want to delete this Composer')) {
        await adminDelete(`/composer-delete/${i}`);
        this.composerData()
    ...
    async cancelCampaign(i) {
      if (await askConfirm('Do You Want to Cancle this Campaign')) {
        await adminPatch(`/email-composer/${i}`, { campaign_status: "cancelled" });
        this.composerData()
    ...
    async resendCampaign(i) {
      if (await askConfirm('Do You Want to Resechdule this Composer')) {
        await adminPost(`/resend-composer/${i}`,);
        this.composerData()
    ...
    async composerData() {
      try {
        const data = await adminGet('/email-composer-list');
        if (data.data.status) {
          this.rows = data.data.data;
        }
      } catch (error) {
