<template>
  <div class="w-full whole flex items-start">
    <div
      class="bg-white border-[2px] border-[#ededed] left-sec mb-[7px]  xl:mb-[30px] rounded-[12px] px-[17px] md:px-[24px] xl:px-[35px] pt-[35px] pb-[18px] flex flex-col ">
      ...
      <div class="space-y-5  pr-2 inner-fields">
        ...
        <div>
          ...
          <div class="flex flex-wrap gap-2 mt-2">
            <div v-for="(email, index) in recipientList" :key="index"
              class="group relative flex gap-2 bg-[#f5f5f5] text-sm text-[#1C1C1E] px-3 py-1 pr-6 rounded-[8px] w-[200px] overflow-hidden whitespace-nowrap text-ellipsis">
              <span class="inline-block truncate max-w-[150px] align-middle">
                {{ email.email }}
              </span>
              <span @click="removeRecipient(index)"
                class="absolute right-2 top-1/2  -translate-y-1/2 text-[#ff5757] cursor-pointer mx-2 hover:font-bold">
                ✕</span>
              <div
        ...
        <div v-if="schedule">
          ...
          <div class="relative">
            <input type="date" v-model="date" placeholder="DD MM YYYY" :min="new Date().toISOString().split('T')[0]"
              class="w-full mt-1 border border-[#E5E5E5] rounded-[12px]  text-sm text-[#999999] outline-none" />
          </div>
        </div>
        <div v-if="schedule">
          ...
          <div class="relative">
            <input type="time" v-model="time" placeholder="00:00"
              :min="date === new Date().toISOString().split('T')[0] ? new Date().toTimeString().slice(0, 5) : null"
              class="w-full mt-1 border border-[#E5E5E5] rounded-[12px]  text-sm text-[#999999] outline-none" />
          </div>
        </div>
      </div>
    ...
    <div class=" md:mb-4 mb-5 right-sec  ">
      ...
      <div class=" border-[2px] mt-[18px]   border-[#ededed]  rounded-[10px]  bg-white h-fit">
        <div
        ...
        <div class="sm:px-3 px-2 h-[460px] md:h-[589px]">
          ...
          <div class="md:h-[509px] h-[380px] relative flex flex-col justify-between border-t border-[#d1d5db]">
            <div class="flex-1  border-t-red overflow-y-auto quill-no-borders">
  ...
  <Sidebar v-model:visible="showSidebar" modal position="right"
    ...
    <div
      class=" flex flex-col pb-[27px] pt-[40px] md:pt-[50px] px-1 sm:px-2 md:px-4 bg-white overflow-y-auto h-screen justify-between">
      <div>
          ...
          <template v-if="csvUploaded" class=" mb-5">
            <div
              ...
              <div class="flex items-center gap-2 pb-2 border-b-[2px] border-[#ededed]"
                ...
                <i class="bi bi-code text-[18px] text-[#676B6C]"></i><span
                  class="md:text-[20px] text-[18px] text-[#282823]">Map Lead
                  CSV
...
export default {
  ...
  methods: {
    async sendTestemail() {
      if (!this.body) {
        return errorAlert("Body is required")
      }
      if (!this.subject) {
        return errorAlert("Subject is required")
      }
    ...
    async campaignDetail(id) {
      try {
        const response = await adminGet(`/email-composer-detail/${id}`);
    ...
    async publishCampaign() {
      ...
      if (this.schedule) {
        if (!this.date || !this.time) {
          errors.push("Schedule date and time are required");
        } else {
