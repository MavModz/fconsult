            ...
            <div
                ...
                <div class="mb-4" v-if="currentTemplateId === 2">
                    ...
                    <div
                        ...
                        <input type="color" v-model="dynamicData.headerBorder" class="w-5 h-5 cursor-pointer" /> -->
                         <input type="color" v-model="dynamicData.headerBorder" class="w-5 h-5  cursor-pointer" />
                        <InputText v-model="dynamicData.headerBorder" readonly class="flex-1 outline-none bg-transparent" />
                    </div>
                </div>
                ...
                <div class="mb-4" v-if="currentTemplateId === 3">
                    ...
                    <div
                        ...
                        <input type="color" v-model="dynamicData.titleColor" class="w-5 h-5 cursor-pointer" /> -->
                         <input type="color" v-model="dynamicData.titleColor" class="w-5 h-5  cursor-pointer" />
                        <InputText v-model="dynamicData.titleColor" readonly class="flex-1 outline-none bg-transparent" />
                    </div>
                </div>
                <div class="mb-4">
                    <div class="flex justify-between mb-1 font-medium text-[17px]">
                ...
                <div v-if="isScheduled">
                    <div class="mb-4">
                        ...
                        <input type="date" v-model="dynamicData.scheduleDate"  :min="new Date().toISOString().split('T')[0]"
                            class="py-[19px] px-[21px] rounded-[12px] w-full border-[1.2px] border-[#E5E5EA]" />
                   <p v-if="validationErrors.scheduleDate" class="text-red-500 text-sm mt-1">{{ validationErrors.scheduleDate }}
                    </p>
                 </div>
                    <div class="pb-6 border-b-[2px] border-[#ededed] mb-4">
                        <label class="block mb-1 text-[17px]  text-medium text-[#1C1C1E]">Time</label>
                        <input type="time" v-model="dynamicData.scheduleTime" :min="date === new Date().toISOString().split('T')[0] ? new Date().toTimeString().slice(0,5) : null"
                            class="py-[17px] px-[21px] rounded-[12px] w-full border-[1.2px] border-[#E5E5EA]" />
                    <p v-if="validationErrors.scheduleTime" class="text-red-500 text-sm mt-1">{{ validationErrors.scheduleTime }}
                    </p>
                </div>
                </div>
                ...
                <button v-else @click="publish" class="w-full p-[14px] rounded-[4px] text-[#FF5757] mt-4 text-[19px] font-semibold bg-[#FF57571A]  flex items-center justify-center gap-2" :disabled="loading">
...
export default {
    ...
    methods: {
        ...
        async campaignDetails() {
            ...
            this.dynamicData = {
                ...
                scheduleDate: new Date(data.scheduleDate).toISOString().split("T")[0],
        ...
        async resendCampaign() {
            ...
            console.log(payload)
            const data = await adminPost('/resend-create-campaign', payload)
            if (data.data.status) {
 ...
 if (this.isScheduled) {
    const { scheduleDate, scheduleTime } = this.dynamicData;
    if (!scheduleDate) {
      errors.push("Schedule date is required");
      this.validationErrors.scheduleDate = "Schedule date is required.";
    } 
     if (!scheduleTime) {
      errors.push("Schedule time is required");
      this.validationErrors.scheduleTime = "Schedule time is required.";
    } 
     if (scheduleDate && scheduleTime){
      ...
      if (selectedDateTime < now) {
        ...
        this.validationErrors.scheduleTime = "Schedule time cannot be in the past.";
      }
    }
  }
            if (errors.length > 0) {
                errorAlert("Please fill all the required fields."); // keep this!
                return false;
            }
            return true;
        },
                ...
                if (this.testEmail) {
                    ...
                    let data = {
                        ...
                        dynamicData: {
                            ...
                            ctaLink:this.dynamicData.ctaLink
                        }
                    }
                    if (this.$route.query.id) {
