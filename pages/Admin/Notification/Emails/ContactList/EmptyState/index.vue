<template>
  <div class="flex justify-content-end mb-3" v-if="allContacts.length > 0">
  ...
  <div
    ...
    <div class="flex flex-wrap gap-[14px] sm:gap-[20px] items-center justify-between">
      ...
      <router-link to="/admin/notification/emails/contactlist/contactgroups"
    ...
    <DataTable :value="filteredContacts" class="rounded-[28px] m-0 border leadabc border-[#E2E8F0]" responsiveLayout="scroll"
      :paginator="true" :rows="10" :selection="selectedUsers"
  ...
  <Sidebar v-model:visible="showSidebar" modal position="right"
    ...
    <div>
      <div class="py-[40px] md:py-[50px] px-2 sm:px-3 md:px-4 bg-white flex flex-col h-screen justify-between">
        <div>
          ...
          <div class="form-group mt-4 col-12 col-lg-12 d-lg-flex flex-lg-column pt-4 pt-lg-0 pb-2">
            <label for="campaign-details" class="required">Group Name</label>
            <div class="select-option">
          ...
          <div class="text-[#9CA3AF] text-[16px] sm:text-[17px] hover:text-[#ff5757] underline flex justify-end mb-3"
            @click="AddGroup = true">+ Add New Group </div>
          <div class="form-group mb-5 col-12 col-lg-12 d-lg-flex flex-lg-column pt-4 pt-lg-0 pb-2">
...
export default {
  ...
  async mounted() {
        activateSubmenu('e1', 'Contact List');
    this.initData();
  ...
  methods: {
    ...
    async submitForm() {
      ...
      let data
      if (this.edit) {
        data = await adminPatch(`/contact-list/${this.edit_id}`, newContact)
      } else {
        data = await adminPost("/contact-list", newContact)
      }
      if (data.data.status) {
    ...
    goToContactGroupPage() {
      this.$router.push('/admin/notification/emails/contactlist/contactgroups');
    },
    ...
    async uploadLead() {
      ...
      const id = await adminPost(`/csv-contact-list`, payload);
      if (id.data.status) {
    ...
    async grouplistData() {
      try {
        const data = await adminGet("/group-contact-list");
    ...
    async addGroup(v) {
      try {
        const data = await adminPost("/group-contact-list", v);
        if (data?.status) {
    ...
    async contactListData() {
      try {
        const data = await adminGet("/all-contact-list")
    ...
    async deleteContact(i) {
      if (await deleteConfirm('Do You Want to delete this Contact')) {
        await adminDelete(`/contact-list/${i}`);
        this.contactListData()
