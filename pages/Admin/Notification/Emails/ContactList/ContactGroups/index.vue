...
export default {
  ...
  methods: {
    ...
    // },
    async groupList() {
      const data = await adminGet("/view-group-list")
      if (data.data.status) {
    ...
    async EditGroup(v) {
      try {
        const data = await adminPatch(`/group-list/${this.group_id}`,v);
     ...
     async deleteGroup(i) {
      if (await deleteConfirm('Do You Want to delete this Group.')) {
        await adminDelete(`/group-list/${i}`);
        this.groupList()
        successAlert('Group Deleted');
      }
    },
    async GroupView(data){
      this.$router.push({path:"/admin/notification/emails/contactlist/emptystate",query:{group_name:data.group_name}})
    }
  },
  mounted() {
    this.groupList()
