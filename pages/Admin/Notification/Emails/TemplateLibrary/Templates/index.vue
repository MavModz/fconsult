    ...
    <div v-if="active == 'Create Template'"
      ...
      <div v-for="(template, index) in templates" :key="index"
        ...
        <div
          ...
          <router-link :to="`/admin/notification/emails/templatelibrary/templatepreview?id=${template.id}`"
...
export default {
  ...
  methods: {
      ...
      return [
        {
          label: 'View',
          command: () => {
            this.openDialogmyTemplate(data)
          },
        },
        {
          label: 'Edit',
    ...
    async TemplateList() {
      const res = await adminGet("/email-template")
    ...
    async deleteTemplate(v) {
      if (await deleteConfirm('Do You Want to delete this Template?')) {
        this.loading = true;
        try {
          const res = await adminDelete(`/email-template/${v._id}`);
          console.log("res", res);
          if (res.status) {
            this.loading = false;
