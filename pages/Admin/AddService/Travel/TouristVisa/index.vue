...
>
  <div class="flex items-center gap-[10px] sm:gap-4 inputs">
    ...
    <div class="input-groups">
      <label>Add User Name</label>
      <input type="text" class="outline-none" maxlength="15" v-model="userName" placeholder="-------------------" />
    </div>
  ...
  methods: {
    ...
    async submitForm(v) {
      ...
      try {
        ...
        // Submit data
        const res = await adminPost('/admin-study-add', v);
    ...
    async editListingForm(v) {
      ...
      try {
        ...
        const res = await adminPut(`/admin-study-update/${this.$route.query.id}`, v);
