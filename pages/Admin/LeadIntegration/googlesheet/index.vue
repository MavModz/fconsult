<template>
  <div v-if="data" class="languages webhook-container">
    ...
    <div class="warning">
      <p>
        <img src="/img/svg/alert.svg" height="18px"  alt="" />
  ...
  <Dialog v-model:visible="sheetModal" modal header="Import Sheet From Google Sheet" :style="{ 'min-width': '50vw' }"
    ...
    <FormKit type="form" @submit="findSheet">
      ...
      <div class="warning">
        <p>
          <img src="/img/svg/alert.svg" h alt="" />
          Allowed Columns: name, email, phone, city, state, country
        </p>
      </div>
      <div class="code flex flex-col gap-3">
        ...
        <FormKit label="Campaign Name" validation="required" type="select" name="campaign" placeholder="Select campaign"
          :options="allCampaign"><template #label> Campaign Name <span style="color: red;">*</span></template></FormKit>
      </div>
  ...
  <Dialog v-model:visible="leadModal" modal header="Preview Leads" :style="{ 'min-width': '75vw' }" :draggable="false">
    ...
    <div class="modal-footer-section">
      <button type="button" class="btn-btn-cancel" @click="leadModal = false">Cancel</button>
...
export default {
  ...
  methods: {
    ...
    async deleteGoogleSheet(i) {
      console.log("i",i)
      const data = await adminDelete(`/google-sheet-delete/${i}`)
      if (data.data.status) {
        this.init()
        successAlert("Successfully delete sheet")
      } else {
