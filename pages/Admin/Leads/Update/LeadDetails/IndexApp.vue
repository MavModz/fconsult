      ...
      <div class="col-12 col-lg-8 d-lg-flex flex-lg-column">
        ...
        <div class="row pt-4">
          ...
          <div class="form-group col-12 col-lg-5 d-lg-flex flex-lg-column pt-4 pt-lg-0">
            ...
            <input v-model="lead.age" class="form-control" maxlength="2" type="number" placeholder="Age" />
          </div>
        </div>
        <div class="row pt-4">
          <div class="form-group col-12 col-lg-7 d-lg-flex flex-lg-column pt-4 pt-lg-0">
            <label for="exampleInputPassword1" class="required">Phone Number</label>
            <input v-model="lead.phone" class="form-control" type="text" placeholder="Phone Number" maxlength="10"
              @input="validatePhoneNumber" />
               <small v-if="phoneError" class="text-red-500">{{ phoneError }}</small>
          </div>
          <div class="form-group col-12 col-lg-5 d-lg-flex flex-lg-column pt-4 pt-lg-0">
            <label for="datepicker" class="required">Email ID</label>
            <input v-model="lead.email" class="form-control" type="email" placeholder="Email ID" @blur="validateEmail"  @paste="removeEmojis($event, 'email')"/>
          <small v-if="emailError" class="text-red-500">{{ emailError }}</small>
         </div>
        </div>
        <div class="row pt-4 pb-4">
          <div class="form-group col-lg-7 d-lg-flex flex-lg-column col-12 pt-4 pt-lg-0">
...
export default {
 ...
 validateEmail() {
      const emailPattern =
        /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/; // simple email regex

      if (!this.lead.email || !emailPattern.test(this.lead.email)) {
