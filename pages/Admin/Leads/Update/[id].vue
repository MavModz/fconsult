  ...
  methods: {
    ...
    async init() {
      await adminGet(`/leads/${this.id}`).then(async (r) => {
        ...
        this.originalLead = JSON.parse(JSON.stringify(this.lead))
      })
    },
    async AddLeads() {
      this.loading = true
      console.log("lead payload",this.lead)
 const isValid = this.$refs.LeadDetails.validateForm();
  if (!isValid) {
    this.loading = false;
    errorAlert("Please fix validation errors before submitting");
    return;
  }
      // if(!this.lead.phone ){
      //   this.loading = false
      //  return  errorAlert("Phone is required")
      ...
      //  }
      if (this.lead.name === null || this.lead.name === undefined || this.lead.name === "") {
        errorAlert("Name is required")
        this.loading = false
        return
      } else if (this.lead.email === null || this.lead.email === undefined || this.lead.email === "") {
        errorAlert("Email is required")
        this.loading = false
        return
      }
      else if (this.lead.phone === null || this.lead.phone === undefined || this.lead.phone === "") {
        errorAlert("Phone is required")
        this.loading = false
        return
      }
       if (JSON.stringify(this.lead) === JSON.stringify(this.originalLead)) {
        this.loading = false
          this.$router.go(-1) 
    return
      }
