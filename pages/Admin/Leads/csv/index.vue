<template>
  ...
  <Sidebar v-model:visible="newSidebar" modal position="right"
    class="w-[32rem] h-full overflow-y-auto   sm:w-[32rem] md:w-[34rem] lg:w-[32rem] xl:w-[32rem] 2xl:w-[34rem]"
    title="CSV Upload Status">
    <div>
      <div class="pt-[55px]  px-3 md:px-[40px] bg-white overflow-y-auto ">
        <div class="flex  justify-between items-center">
          <h2 class="text-[28px] text-[#1F1B2D] mt-[16px] font-medium">Upload From CSV</h2>
          <button @click="newSidebar = false" class="mt-[-50px] text-[#d8d8d8] text-[43px]">&times;</button>
        </div>

        <div class=" mt-[29px] input-group">
          <label class="required">Upload Lead CSV File</label>
          ...
          <p class="mb-[6px] text-[#8d6d16]">[Name, Email, Phone is required.]</p>
        </div>
        ...
        <div class="form-group col-12 col-lg-12 d-lg-flex flex-lg-column pt-4 pt-lg-0 pb-4">
          <label for="campaign-details" class="required">Campaign</label>
          <div class="select-option">
          <select v-model="campaign_name" class="form-select border-none appearance-none focus:outline-none focus:ring-0 outline-none focus-outline-none custom-select">
         <option value="" disabled>Select campaign</option>
          ...
          <router-link to="/admin/campaign">
           <div class="flex justify-content-end">
                <button class=" text-[#909090] text-[18px] hover:text-[#ff5757] hover:underline mt-1" >+ Add Campaign</button>
          </div>
          </router-link>
        </div>
        <template v-if="csvUploaded" class="mb-5">
          <div class="border-[2px] p-[19px] pb-[24px] border-[#EFECF3] rounded-[16px] mb-[26px]">
            ...
            <div class="border border-[#E2E8F0] mt-4 rounded-[30px] overflow-hidden w-full " v-if="csvLeadData">
              <table class="text-[17px] w-full table-fixed">
                ...
                <thead class="bg-[#F8FAFC] text-gray-600 text-[17px]">
                  <tr>
                    <th class=" w-[42%]  py-[16px] px-[30px] text-[#64748B] text-center font-semibold w-1/2">
                      Column
                    </th>
                    <th class="w-[58%]  py-[16px] px-[30px] text-[#64748B] text-center font-semibold w-1/2">
                      Value
                    </th>
                  </tr>
                </thead>
                ...
                <tbody>
                  <tr v-for="(field, index) in requiredFields" :key="index" class="border-t">
                    <td class="py-[18px] px-[28px] text-[#3c3c3c] w-1/2">
                      {{ field.label }}
                    </td>

                    <!-- Dynamic cell: shows dropdown or selection summary -->
                    <td class="py-[18px] px-[28px] w-1/2">
            ...
            <div v-if="addtionalActive">
              <div class="border border-[#E2E8F0] mt-4 rounded-[30px] overflow-hidden w-full ">
                ...
                <table class="text-[17px] w-full table-fixed">
                  <thead class="bg-[#F8FAFC] text-gray-600 text-[17px]">
                    <tr>
                      <th class="w-[42%] py-[16px] px-[30px] text-[#64748B] text-center font-semibold">Column</th>
                      <th class=" w-[50%] py-[16px] px-[30px] text-[#64748B] text-center font-semibold border-l">Value</th>
                      <th class="w-[8%]"></th>
                    </tr>
                  </thead>
                  <tbody>
        ...
        <button
          class="w-full bg-[#ff5757] text-white  py-[10px] md:py-[19px] rounded-[21px] font-semibold text-[18px] md:text-[19px]"
          @click="uploadLead">
          Upload Lead
        </button>
      </div>
    ...
    async uploadLead() {
      ...
      for (const row of this.csvData) {
        ...
        for (const field of this.requiredFields) {
          ...
          mapped[field.label.toLowerCase()] = value;
        }

        for (const field of this.extraFields) {
          if (field.label && field.value) {
            mapped[field.label.toLowerCase()] = row[field.value];
          }
        }
      ...
      const id = (await adminPost(`/lead-csv`, data));
      if(id.data.status){
        successAlert("Upload csv successfully!")
      }
