    ...
    <div class="lead-detail row px-5 pb-3">
      <div class="row">
        <div class="input-field col-12 col-lg-2 d-lg-flex flex-lg-column pt-4 pt-lg-0 px-2">
          ...
          <input 
  :value="formatIndian(invoice.total_amount)" 
  type="text" 
  disabled 
  placeholder="Rs. 24,899 /-" 
/>
          <!-- <input v-model="invoice.total_amount" type="text" disabled placeholder="Rs. 24,899 /-" @keyup="calculate()" @keypress="restrictToNumbers"/> -->
        </div>
        <div class="form-group col-12 col-lg-3 d-lg-flex flex-lg-column pt-4 pt-lg-0">
          <label for="payment">Payment Mode</label>
          <select v-model="invoice.payment_method" @change="updatePaymentMethod">
        ...
        <div v-if="invoice.payment_type === 'full'"
          class="form-group col-12 col-lg-3 d-lg-flex flex-lg-column pt-4 pt-lg-0">
    ...
    <div class="lead-setting d-flex justify-content-between align-items-center gap-3 flex-wrap">
      <div class="d-flex justify-content-between gap-4">
        ...
        <div class="form-group">
          <select v-model="invoice.emi_duration" @change="updateEmi(); calculate()">
            <option v-for="i in 20" :value=i>{{ i }} Months</option>
          </select>
        </div>
      </div>
      <div class="remaining-amount">
        <h4><span>Remaining Amount </span><span style="color: #FF5757;">Rs {{ remaining }}/-</span></h4>
      </div>
    </div>
    <div class="lead-detail row px-5 pb-3 mt-3">
      <div class="row" v-for="i in invoice.emi">
        <div class="input-field col-12 col-lg-2 d-lg-flex flex-lg-column pt-4 pt-lg-0 px-2">
          ...
          <input v-model="i.amount" @change="calculate()" type="text" placeholder="Rs. 24,899 /-" @keypress="restrictToNumbers"/>
        </div>
  ...
  methods: {
    updateEmi() {
      ...
      for (let i = 0; i < this.invoice.emi_duration; i++) {
        this.invoice.emi.push({ method: this.invoice.payment_method, status: "unpaid", date: Date.now(), amount: 0, reference: null, remaining: 0 })
      }
    },
    calculate() {
      this.remaining = this.invoice.total_amount - this.invoice.emi.reduce((sum, item) => sum + parseFloat(item.amount), 0)

      if (this.remaining < 0) {
        errorAlert("Amount is in negative")
      }
    },
     formatIndian(amount) {
    ...
    async updatePaymentMethod(){
      for(let i in this.invoice.emi){
        this.invoice.emi[i].method=this.invoice.payment_method
      }
    },
