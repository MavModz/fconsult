<template>
  <div class="py-[29px] px-[32px] bg-white rounded-xl shadow mt-[28px] mb-5">
    <div class="flex justify-between items-center mb-4 border-b-[2px] pb-3 border-[#ededed]">
      <h2 class="text-[21px]  text-[#282823] font-semibold">Unpaid Invoice</h2>
      <div @click="this.$router.push('/admin/invoice')"  class="text-[#6B7280] d-flex align-items-center gap-1  fw-medium  text-[15px] cursor-pointer">
...
export default {
  ...
  methods: {
      ...
      return [
        {
          label: 'View',
          command: () => {
            this.viewInvoiceDetails(slotProps)
          },
        }
      ];
    },
    ...
    async invoiceTable() {
      try {
        const data = await adminGet("/customer-dashboard-unpaid-invoice")
        if (data.data.status) {
    ...
    viewInvoiceDetails(data){
      ...
      if(data.data.payment_type == 'full'){
        this.$router.push(`/admin/invoice/component/fullpayment/${data.data._id}`)
      }else{
        this.$router.push(`/admin/invoice/component/emi/${data.data._id}`)
