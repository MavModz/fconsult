...
const previousSelection = { ...finalSelection.value }; // Save the previous selection

  // Show the confirmation dialog to update the status
  const confirmed = await showStatusChangeConfirmationDialog(selectedCategory.value, sub);

  // If the user clicks "Cancel", don't change the final selection, just return to the previous one
  if (!confirmed) {
    // Revert to the previous selection
    finalSelection.value = previousSelection;
     dropdownOpen.value = false; // 👈 Close dropdown on cancel
    selectedCategory.value = null; 
  ...
  selectedCategory.value = null;
  subcategories.value = [];
  updateStatus(finalSelection.value.category, finalSelection.value.subcategory); // Proceed with status update
}

async function showStatusChangeConfirmationDialog(status, subStatus) {
  try {
...
function resetFields() {
  finalSelection.value = { category: null, subcategory: null };  // Reset finalSelection
  selectedCategory.value = null;  // Reset selected category
  subcategories.value = []; // Reset subcategories list
  dropdownOpen.value = false;  // Close dropdown
}
// Load from API
async function loadStatuses() {
  try {
    const res = await adminGet('/leadstatus')
...
//       await adminPut(`/update-leads-status/${route.params.id}`, {
//         lead_status: status,
...
async function updateStatus(status, subStatus) {
  try {
    // Proceed with the status update if the user clicks "Update"
    if (status === "Converted Lead") {
      handleConvertedLead();
    } else {
      await adminPut(`/update-leads-status/${route.params.id}`, {
...
async function handleConvertedLead() {
  ...
  try {
    const leadDetails = await adminGet(`/leads/${route.params.id}`);
...
async function convertCustomer(v) {
  loading.value = true
  try {
    await adminPut(`/assign-leads/${route.params.id}?auto=${leadAutomation.value}`, v)
