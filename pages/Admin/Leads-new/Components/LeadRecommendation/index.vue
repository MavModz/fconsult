<template>
  <div class="r-container mt-[32px]" v-if="table">
    <div class="top-bar  flex-wrap">
      ...
      <button class="addDocumnetButton" @click="showRecommendationDialog = true">+ Add Recommendation</button>
    </div>
        ...
        <DataTable class=" mt-3  recommend-t leadabc gallery-container  overflow-hidden " :value="recommendations" 
          ...
          <Column field="status" header="Status" style="max-width: 21rem;" class="text-center table-stat">
            <template #body="{ data }">
              ...
              <span v-if="data?.selected" class="  flex justify-center recmd-tbl-sttss-sts">
                <p v-if="!data?.finalized">Selected</p>
                <p v-else class="w-[120px] text-center  items-center flex justify-center rounded-full text-[#73AA27] p-2 bg-[#EBF6EB]">Finalized</p>
...
export default {
  ...
  methods: {
    ...
    async fetchServices() {
      try {
        const response = await adminGet('/grouped-service-subservice');
    ...
    async addComments(v) {
      ...
      try {
        await adminPost(`/add-comments-recommendation/${this.currentRecommendationId}`, v);
        this.commentCallmodal = false;
        this.viewcomments = true;
