<template>
  ...
  <div class=" recommend-table rounded-xl overflow-hidden">
    ...
    <DataTable  v-if="data && data.length > 0" class=" mt-3  recommend-t leadabc gallery-container  overflow-hidden " :value="data" paginator :rows="10"
      :rowClass="() => ''">
      ...
      <Column field="status" header="Status" style="max-width: 21rem;" class="text-center table-stat">
        <template #body="{ data }">
          ...
          <span v-if="data?.selected" class=" flex justify-center recmd-tbl-sttss-sts">
            ...
            <p v-else class="w-[120px]   items-center flex justify-center rounded-full  p-2  text-light bg-[#ff5757]">Finalized</p>
