<template>
  <Sidebar v-model:visible="addDocSidebar" header="" position="right"
    :style="{ width: '520px', 'overflow': 'hidden scroll' }" class="relative pt-5">
    <div class="d-flex justify-between mt-[10px] mb-[20px]">
				<span class="text-[#000] medium text-[30px]">Add Documents</span>
        ...
        <div>
          ...
          <div class="mt-5" v-if="showChecklistDialog">
            ...
            <div class="flex justify-end text-[#909090] font-medium text-[18px] underline mt-2 cursor-pointer"
              @click="$router.push('/admin/settings/LeadManagement#checklist-section')">
  ...
  <Dialog v-model:visible="showUploadDialog" @update:visible="handleDialogClose" modal header="Upload Document"
    ...
    <div class="space-y-5">
      <div>
        <label class="block font-medium text-[20px] text-[#0F172A] mt-3 mb-1 required">Select Category</label>
      ...
      <div>
        <div class="input-group" v-if="is_documentNameVisible">
          <label class="block text-[20px] font-medium text-[#0F172A] mt-2 mb-1 required">Document Name</label>
        ...
        <div class="text-[16px] text-[#676B6C] mb-[30px] mt-2 justify-end flex items-center gap-3">
         ...
         <p v-if="errors.documentName" class="text-red-500 text-sm mt-1">{{ errors.documentName }}</p>
      </div>
...
export default {
  ...
  methods: {
    ...
    // },
    async UploadsubmitForm() {
       this.errors = { category: "", documentName: "", files: "" };
      const categories = Object.keys(this.uploadedFilesByCategory);
       let isValid = true;
    ...
    if (!isValid) {
      return errorAlert("Please fill all the required fields before submitting.");
    }
      //  if (!this.selectedCategory) {
      //   errorAlert("Please select a category and upload at least one file.");
      //   return;
      // }
      // if (categories.length === 0) {
      //   return errorAlert("Please select a category and upload at least one file.");
      ...
      // if (
      //   categories.length === 1 &&
      //   this.uploadedFilesByCategory[categories[0]]?.length === 0
      // ) {
      //   return errorAlert("Please upload a file before submitting");
      // }
      // if (
      //   categories.length === 1 &&
      //   this.uploadedFilesByCategory[categories[0]]?.length === 1 &&
