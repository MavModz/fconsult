<template>
  <Dialog v-model:visible="addModalDialogue" :style="{'min-width': '380px'}" modal header="Update Video">
    <section>
      <FormKit type="form" @submit="submitForm">
        <div class="upload-image-box upload-video">
          ...
          <!-- <label class="image-upload-container rounded-2xl my-3 shadow-lg cursor-pointer" data-kt-image-input-action="change"
            ...
            <input ref="update_icon_imageInput" id="update_icon_imageInput" type="file" accept="image/png, image/jpeg, image/jpg" style="display: none"
          ...
          <!-- Outer container with relative position -->
<div class="relative  my-3">

  <!-- Background label with image -->
  <label
    class="image-upload-container rounded-2xl shadow-lg block w-[200px] bg-cover h-[200px] bg-cover bg-center cursor-default"
    data-kt-image-input-action="change"
    :style="'background-image: url(' + update_icon_url + ')'"
  >
    <!-- Hidden file input -->
    <input
      ref="update_icon_imageInput"
      id="update_icon_imageInput"
      type="file"
    ...
    <!-- Hidden input for v-model binding -->
    <input type="text" v-model="update_icon_url" id="icon" hidden />
  </label>

  <!-- Edit Icon on top-right corner -->
  <span
    class="absolute top-2 right-2 bg-white px-2 py-1 rounded-full shadow cursor-pointer z-10"
    @click="$refs.update_icon_imageInput.click()"
  >
    <BootstrapIcon name="pencil-square" class="text-gray-700" />
  </span>

  <!-- Hidden FormKit input -->
  <FormKit v-model="update_icon_url" type="hidden" name="thumbnail" />
</div>
