    ...
    <div class='joblis-basic-header'>
      ...
      <div class='joblis-basic-basdgn'>
        ...
        <div class='jpblis-jbspct'>
         ...
         <span class='jblis-jb-possnr'> {{formData?.basicInfoData?.position}}</span><br/>
         <span class='jblis-jb-post'>Company name:</span>
         <span class='jblis-jb-possnr'> {{formData?.basicInfoData?.title}}</span><br/>
         <span class='jblis-jb-post'>Location:</span>
         <span class='jblis-jb-possnr'> {{formData?.basicInfoData?.country}}</span>
        </div>
     <div class='joblis-bacs-2stbx'>
      <div class="mb-1 jobliscmp-dro">
        <label  class='required joblis-basc-lble'>Job category</label>
        <div class="">
            ...
            {{ validationError.jobCategory_name }}
              ...
              <div class="mb-1 jobliscmp-dro">
                <label  class='required joblis-basc-lble'>Employment type</label>
                <div class="">
...
<label  class='required joblis-basc-lble-sal'>Salary range</label>
     <div class='joblis-bacs-4stbx'>
      <div class="joblis-currency">
      ...
      <div class='job-dtls-salry-range'>
        <input v-model="listService.salary_range.start" type= 'text ' placeholder='15,00' class='joblis-basc-inpt1st'@keypress="restrictToNumbers"/>
      ...
      <div class='job-dtls-salry-range'>
        <input v-model="listService.salary_range.end" type= 'text ' placeholder='2,000' class='joblis-basc-inpt1st'@keypress="restrictToNumbers"/>
     ...
     <div class='steper-jobdtls-quilledtr'>
        <label class='custom-job-description required joblis-basc-lble-sal'>Job description</label>
        <QuillEditor v-model="listService.overview" :modules="modules" ref="qhtml" class="mb-4"  @text-change="onEditorInput"/>
    ...
    <FileName :listService="listService" :error="validationError.document_url"/>
     ...
     <div class="mb-2 relative mt-[50px]">
        <label class=" text-[#282823] text-[20px]  mb-3 flex justify-between" for=""><span class="required">Add Tab</span><span class="cursor-pointer"
...
export default {
  ...
  async mounted() {
    ...
    if (this.$route.query.id && this.listService.tab.length > 0) {
      this.tempTabs = this.listService.tab
    }

  },
  watch: {
  'listService.jobCategory_name'(val) {
    if (val) this.validationError.jobCategory_name = '';
  },
  'listService.employement_type'(val) {
    if (val) this.validationError.employement_type = '';
  },
  'listService.salary_range.currency'(val) {
    if (val) this.validationError.currency = '';
  },
  'listService.salary_range.start'(val) {
    if (val) this.validationError.start = '';
  },
  'listService.salary_range.end'(val) {
    if (val) this.validationError.end = '';
  },
  'listService.salary_range.duration'(val) {
    if (val) this.validationError.duration = '';
  },
  'listService.document_url'(val) {
    if (val) this.validationError.document_url = '';
  },
 
  'listService.tab'(val) {
    if (val.length > 0) this.validationError.tab = '';
  }
},
  methods: {
    onEditorInput() {
  const html = this.$refs.qhtml.getHTML().trim();
  this.listService.overview = html;

  const plainText = this.stripHtml(html).trim();
  const charCount = plainText.length; // count characters (including spaces)

  if (charCount === 0) {
    this.validationError.overview = 'Description is required.';
  } else if (charCount < 150) {
    this.validationError.overview = 'Description must be at least of 150 characters.';
  } else {
    this.validationError.overview = '';
  }
},
    ...
    validateFields() {
      ...
      if (!this.listService.jobCategory_name) {
        this.validationError.jobCategory_name = "Job Category is required";
      }
      ...
      if (!this.listService.salary_range?.start) {
        this.validationError.start = "Salary start range is required";
      }
      if (!this.listService.salary_range?.end) {
        this.validationError.end = "Salary end range is required";
      }
      ...
      if (!this.listService.document_url) {
        this.validationError.document_url = "Image is required";
      }
      ...
      //   this.validationError.overview = 'Description must be at least 30 words.';
      // }
      if (charCount === 0) {
  this.validationError.overview = 'Description is required.';
} else if (charCount < 150) {
  this.validationError.overview = 'Description must be at least of 150 characters.';
    ...
    showMainTab() {
      if (this.$route.query.id) {
        this.listService.tab = this.tempTabs;
      }
      else
        if (this.tempTabs.length > 0) {
         ...
         if (this.listService.tab.length > 0) {
    this.validationError.tab = '';
  }
    },
    createTab() {
      this.tabObj = {
