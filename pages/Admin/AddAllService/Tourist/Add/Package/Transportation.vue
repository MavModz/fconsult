...
export default {
	...
	data() {
		return {
			...
			alltempUser: [],
			visibleRight: false,
			localTransportation: JSON.parse(JSON.stringify(this.x))
		}
	},
	watch: {
		x: {
			handler(newVal) {
				this.localTransportation = JSON.parse(JSON.stringify(newVal));
			},
			deep: true,
			immediate: true
		},
		localTransportation: {
			handler(newVal) {
				this.$emit('update:x', newVal);  // emit changes to parent
			},
			deep: true
		}
	},
	mounted() {
		console.log("under transportation x", this.x)
	},
	methods: {
