        ...
        <div class="tourist-basic-1st">
          ...
          <div class="tourist-bacs-1stbx">
            <div class="tourist-bsc-dic-wd">
              ...
              <FormKit type="text" validation="required" placeholder="Title" name="title" class="tourist-basc-inpt1st"
                validation-label="Destination Title" />
            </div>
            <div class="tourist-bsc-dic-wd">
            ...
            <div class="col-sm-6 mb-3 mb-sm-0">
              <div class="">
                ...
                <FormKit validation="required" v-model="selectedcountry" type="select" name="country"
                  :options="allCountry" placeholder="Select a Country" @change="getState()"
                  validation-label="Country selection" />
              </div>
            </div>
            <div class="col-sm-6 mb-3 mb-sm-0">
              <div class="input-field">
                ...
                <FormKit validation="required" type="text"  class="form-control py-3"
                  name="pincode" placeholder="Enter your PIN Code" maxlength="6" @keypress="restrictToNumbers"
            ...
            <div class="col-sm-6 mb-3 mb-sm-0">
              <div class="mb-3">
                ...
                <FormKit validation="required" v-model="selectedstate" type="select" name="state" :options="allState"
                  placeholder="Select a State" @change="getCity()" />
              </div>
            </div>
            <div class="col-sm-6 mb-3 mb-sm-0">
              <div class="mb-3">
          ...
          <div class="input-field">
            ...
            <FormKit validation="required" name="destination_address" type="text" placeholder="Destination Address"
              validation-label="Destination" />
          </div>

          <div class="input-field">
            <label for="field-name">Description</label>
            <FormKit validation="required" name="description" as="textarea" placeholder="Add description here"
              validation-label="Description" />
          </div>

          <div class="input-field">
            <label for="field-name">Price</label>
            <FormKit validation="required" name="price" type="number" placeholder="Enter price"
              validation-label="Price"  @keypress="restrictToNumbers"/>
          </div>
                ...
                <div v-if="cover_image" class="upload-image-after">
                  ...
                  <label data-kt-image-input-action="change">
                    ...
                    <FormKit validation="required" name="cover_image" type="text" v-model="cover_image" id="icon"
                      validation-label="Cover Image" hidden />
                  </label>
                  <!-- </div> -->
                </div>
  ...
  methods: {
    ...
    async submitForm(v) {
      ...
      // } 
        ...
        v.tourist.destination_address = v.destination_address

        if (image) {
          v.media_list = await Promise.all(
            image.map(async (img) => ({
              media_url: await uploadImageAdminE2(img, `tourist_visa_cover_image_${Date.now()}`),
              alt_tag: "media_tourist"
            }))
          );
        ...
        await adminPost(`/admin-study-add`, v).then((response) => {
          this.loading = false;
          this.tabs = []
          this.formData.days = []
          this.$router.push({
