      ...
      <div class="input-field">
        <div class="d-flex flex-wrap gap-x-[35px] gap-y-[20px] achiever-list">
          ...
          <div class="tourist-media-size">
            <img class="h-[100%] w-[100%] object-cover rounded-xl" :src="i.media_url" style="height: 150px" />
          </div>
          <div class="tooltip-gallery-cont-blog absolute top-[15px] right-[10px]">
            <SplitButton icon="none" dropdownIcon="pi pi-check" :model="generateMenuItems(index)" />
          </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  mounted() {
    console.log("allimages",this.all_images)
   },
  methods: {
    ...
    async icon_uploadImage() {
      ...
      this.all_images.push({media_url: await uploadImageAdminE2(this.icon_croppedDataUrl, `tourist_visa_cover_image_${Date.now()}`),
