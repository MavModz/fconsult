    ...
    <div class="bg-white px-[20px] py-[20px] flex flex-col gap-4 rounded-xl shadow-md">
      ...
      <div class="mb-0.5 relative">
        ...
        <input type="text" v-model="listService.location"
          class="mx-auto border-upload text-[#9691A4] text-[25px] outline-none pr-[40px!important] rounded-[11px!important] font-[400] w-[90%!important"
          placeholder="Address" />
      ...
      <!-- <div class="bg-white rounded-2xl px-4 py-[30px]">
        ...
        <div class="mb-4  mt-4  relative">
          <input type="text"  v-model="listService.location"
            ...
            alt="" /><span @click="fetchLocation" class="flex items-center gap-[10px] absolute right-0 cursor-pointer"><img src="/img/svg/location.svg"
              alt="" /><span>auto detect</span></span>
        </div>
      </div> -->

      <div class="mb-4 relative bg-white px-[30px] py-[20px] rounded-2xl h-[-webkit-fill-available]">
...
export default {
  data() {
    return {
      ...
      icon_url: null, editorInstance: null, // To store the Quill instance
    };
  },
  components: {
  ...
  async mounted() {
    activateSubmenu('b3', 'Study Visa', true);
  ...
  methods: {
    validateYouTubeUrl() {
      ...
      if (!youtubeRegex.test(url)) {
        this.validationErrors.uploadVideo = 'Please enter a valid YouTube URL.';
      } else {
        this.validationErrors.uploadVideo = '';
      }
    },
    async fetchLocation() {
    ...
    validateFields() {
      ...
      if (!this.listService.uploadVideo) {
        this.validationErrors.uploadVideo = 'Video URL is required.';
      }
    ...
    async handleSubmitService(listService, v) {
      ...
      try {
        ...
        if (res.status === true) {
          ...
          this.resetForm();
          this.$router.push({
            path: '/admin/ViewAllService/StudyList',
