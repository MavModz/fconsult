...
<template>
  <div class="px-[15px] flex justify-between gap-2 flex-wrap rounded-[4px] items-center">
    ...
    <NuxtLink to="/admin/AddAllService/VisitorSer">
  ...
  <div 
    ...
    <div v-for="(i, index) in filterAllListing" class="px-[20px] py-[20px] flex flex-col gap-4 rounded-[15px] shadow">
      <div class="single-listng-srvstdy relative">
        <img class="rounded-xl" :src="i.visitor.cover_image+`?t=${new Date().getDay()}`" alt="" />
...
export default {
  ...
  async mounted() {
    activateSubmenu('b3', 'Visitor Visa Service Lists',true);
  ...
  methods: {
    ...
    async EditListing(i) {
      this.$router.push(`/admin/UpdateListserv/VisitorSer/${i}`)
    },
