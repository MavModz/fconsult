...
<template>
  <div class="px-[15px] flex justify-between gap-2 flex-wrap rounded-[4px] items-center">
    ...
    <NuxtLink to="/admin/AddAllService/JobService">
  ...
  <div
    ...
    <div v-for="(i, index) in filterAllListing" class="px-[20px] py-[20px] flex flex-col gap-4 rounded-[15px] shadow">
      <div class="single-listng-srvstdy relative">
        <img class="rounded-xl" :src="i.work.cover_image+`?t=$`" alt="" />
        ...
        <!-- <div class="flex flex-wrap items-center gap-2 absolute bottom-[10px] right-[20px]" data-bs-toggle="offcanvas"
          ...
          <span class="text-[#F4F7FE] font-normal">Offer</span>
        </div> -->
      ...
      <div class="w-full">
        <div class="pb-[8px]">
          <p class="text-[#454056] text-[18px] font-[700] mb-[10px]">{{ i?.work?.position }}</p>
          <span class="text-[#666276] text-[14px] font-[400]" v-html="i.overview"></span>
        </div>
        <div class="d-flex flex-wrap justify-between">
          <div class="d-flex gap-x-[4px]">
            <!-- <img :src="i?.work?.logo" alt=""> -->
            <span class="text-[15px]">{{ i?.title.split('').length>4 ? i?.title.split('').slice(0,4).join('')+ '...' : i?.title}}</span>
            <span><span><i class="bi bi-geo-alt"></i></span> {{ i?.country }}</span>
          </div>
          <div class="d-flex gap-x-[6px]">
            <p class=""><span class="text-[#ff5757] text-[20px] font-bold">{{ i?.work?.salary_range?.currency }}</span><span class="text-[18px]"> {{ i?.work?.salary_range?.start }}</span></p>
            <span class="font-bold">-</span>
            <p class=""><span class="text-[#ff5757] text-[20px] font-bold">{{ i?.work?.salary_range?.currency }}</span><span class="text-[18px]"> {{ i?.work?.salary_range?.end }}</span></p>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  async mounted() {
    activateSubmenu('b3', 'Work Visa Service Lists',true);
  ...
  methods: {
    ...
    async EditListing(i) {
      this.$router.push(`/admin/UpdateListserv/JobService/${i}`)
    },
