...
<template>
  ...
  <div class="px-[15px] flex justify-between gap-2 flex-wrap rounded-[4px] items-center">
    ...
    <NuxtLink to="/admin/AddAllService/PrService">
  ...
  <div class="bg-[#f9f9f9] h-[70vh] rounded-xl" >
    ...
    <div v-else
      class="px-[20px] py-[20px] mt-[20px] grid grid-cols-3 max-[1150px]:grid-cols-2 max-[768px]:grid-cols-2 max-[500px]:grid-cols-1 gap-6 mx-auto">
      <div v-for="(i, index) in filterAllListing" class="px-[20px] py-[20px] flex flex-col gap-4 rounded-[15px] shadow">
        <div class="single-listng-srvstdy relative">
          <img class="rounded-xl" :src="i?.pr?.cover_image + `?t=${new Date().getDay()}`" alt="" />
...
export default {
  ...
  async mounted() {
    activateSubmenu('b3', 'PR Visa Service Lists', true);
  ...
  methods: {
    ...
    async EditListing(i) {
      this.$router.push(`/admin/UpdateListserv/prService/${i}`)
    },
