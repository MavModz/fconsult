  ...
  <div class="mt-[20px] grid grid-cols-2 max-[1150px]:grid-cols-1 max-[500px]:grid-cols-1 gap-6 mx-auto relative">
    <div class="bg-white px-[20px] py-[20px] flex flex-col gap-4 rounded-xl shadow-md">
      ...
      <div class="col-sm-12 mb-sm-0">
        ...
        <div class="formkit-outer">
          <div class="formkit-wrapper">
            <select class="formkit-input" v-model="listService.country" name="country" required
              aria-required="true" @click="fetchAdminCountries('Permanent Residency')">
      ...
      <div class="d-flex gap-[10px] justify-end">
        ...
        <button type="submit" @click="handleSubmitService(listService, tabObj)"
          class="text-[#fff] bg-[#ff5757] px-[22px] py-[6px] rounded-[8px]">Submit</button>
      </div>
    </div>
    <div class="bg-white px-[20px] py-[20px] flex flex-col gap-4 rounded-lg max-h-[1296px] overflow-y-scroll">
      <div class="mb-4 relative">
        <label class="text-[#282823] text-[20px] font-bold mb-3 flex justify-between" for="">
          ...
          <img data-bs-toggle="offcanvas" data-bs-target="#staticBackdrop1" src="/img/svg/add.svg" alt="" class="cursor-pointer"/>
        </label>
...
export default {
  ...
  async mounted() {
    activateSubmenu('b3',  'Permanent Residency',true);
    this.singleListindId = this.$route.params.id;
    await this.updateFunctn();
    await this.AllAchiever();

    await this.fetchAdminCountries('Permanent Residency');
  ...
  methods: {
    ...
    async AllAchiever() {
      const res = await adminGet(`/company-achievers`)
      this.AchivrListShow = res.data.data
    },
    async updateFunctn() {
      try {
        if (this.singleListindId) {
          ...
          this.listService = {
            ...
            coverImage: data.pr?.cover_image || '',
            collegeLogo: data.pr?.logo || '',
            uploadVideo: data.pr?.video_url || '',
            videoTitle: data.pr?.video_title || '',
            ...
            coordinates: data.pr?.location?.coordinates || [],
            tab: data.pr?.tabs?.map(tab => ({
              name: tab.tabname || '',
              description: tab.overview || '',
            })) || [],
          };
          // this.tempTabs = data?.pr?.tabs?.map(tab => ({
          //   name: tab.tabname || '',
          //   description: tab.overview || '',
          // }))
          this.AllImagePic = data.pr?.images || [];
    ...
    validateFinalFields() {
      ...
      if (!this.listService.title.trim()) {
        this.validationFinalErrors.title = 'Title is required.';
      }
      if (!this.listService.location.trim()) {
        this.validationFinalErrors.location = 'Location is required.';
      }
      if (!this.listService.city.trim()) {
        this.validationFinalErrors.city = 'City is required.';
      }
      if (!this.listService.uploadVideo.trim()) {
        this.validationFinalErrors.uploadVideo = 'video is required.';
      }
      if (!this.listService.videoTitle.trim()) {
        this.validationFinalErrors.videoTitle = 'videoTitle is required.';
      }
    ...
    async handleSubmitService(listService, v) {
      ...
      const payload = {
        ...
        service_name: 'Permanent Residency',
        type: 'pr',
        pr: {
      ...
      try {
        this.loading = true;
        const res = await adminPut(`/company-listing/${this.singleListindId}`, payload);
        if (res.status === true) {
          this.resetForm();
          this.$router.push({
            path: '/admin/ViewAllService/PrListing',
