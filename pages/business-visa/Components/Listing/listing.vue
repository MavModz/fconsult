<template>
  <div class="offers-container  customclass-listing-container">
    <div
      class="d-flex flex-wrap justify-content-between gap-2 px-xl-5 px-lg-4 px-md-3 px-sm-2 px-xs-1 py-2"
    >
      <div class="d-flex flex-wrap gap-x-8 gap-y-4 items-center">
        <h2 style="margin-bottom: 0px" class="study-newabr-head text-[24px] sm:text-[24px] md:text-[26px] lg:text-[30px]">
          Business Abroad Consultants
        </h2>
        <span
          v-for="(i, index) in countries_Service"
          :key="index"
          class="countries_newservice"
          tabindex="0"
          @click="filterCountry(i)"
          >{{ i }}</span
        >
      </div>

      <div class="d-flex align-items-center gap-3">
        <span @click="goToSearch('business-listing', 'Business Visa')"  class="ms-4 cursor-pointer"
          >View all  <i class="bi bi-arrow-right w-4"></i>
        </span>
      </div>
    </div>
        ...
        >
          ...
          >
            ...
            <div
              ...
              class="accomodation carousel-inner"
            >
            ...
            <div
              ...
              class="items-center gap-2 px-2 w-[-webkit-fill-available]"
            >
              <NuxtLink  :to="item?.state.replaceAll(' ','-') + '/immigration-company/' + item.company_name?.replaceAll(' ', '-') + '/' + item?.landmark?.replaceAll(' ','-') + '/f&c-' + item?._id + '/about-us'" class="flex items-center gap-1 w-[70px]">
              ...
              <NuxtLink  :to="item?.state.replaceAll(' ','-') + '/immigration-company/' + item.company_name?.replaceAll(' ', '-') + '/' + item?.landmark?.replaceAll(' ','-') + '/f&c-' + item?._id + '/about-us'" class="flex items-center gap-2">
                <img src="/img/svg/grey-map.svg" alt="Location" />
                <span
                  class="text-left text-[16px] max-[991px]:text-[15px] max-[767px]:text-[14px] text-[#666276]"
                >
...
export default {
  ...
  methods: {
    ...
    async fetchData(country = '') {
      try {
        const url = country
          ? `/home-review-companies-all/Business Visa/${country}`
          : `/home-review-companies-all/Business Visa`;
    ...
    async countriesServices() {
      try {
        const response = await homeGet(
          `/home-review-Listing-country/Business Visa`,
        );
        if (response.status) {
